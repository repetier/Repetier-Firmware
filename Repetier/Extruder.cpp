...
#ifdef TEMP_PID
        ,0,EXT0_PID_INTEGRAL_DRIVE_MAX,EXT0_PID_INTEGRAL_DRIVE_MIN,EXT0_PID_P,EXT0_PID_I,EXT0_PID_D,EXT0_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif 
    }
...
#ifdef TEMP_PID
        ,0,EXT1_PID_INTEGRAL_DRIVE_MAX,EXT1_PID_INTEGRAL_DRIVE_MIN,EXT1_PID_P,EXT1_PID_I,EXT1_PID_D,EXT1_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif
    } 
...
#ifdef TEMP_PID
        ,0,EXT2_PID_INTEGRAL_DRIVE_MAX,EXT2_PID_INTEGRAL_DRIVE_MIN,EXT2_PID_P,EXT2_PID_I,EXT2_PID_D,EXT2_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif
    } 
...
#ifdef TEMP_PID
        ,0,EXT3_PID_INTEGRAL_DRIVE_MAX,EXT3_PID_INTEGRAL_DRIVE_MIN,EXT3_PID_P,EXT3_PID_I,EXT3_PID_D,EXT3_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif
    } 
...
#ifdef TEMP_PID
        ,0,EXT4_PID_INTEGRAL_DRIVE_MAX,EXT4_PID_INTEGRAL_DRIVE_MIN,EXT4_PID_P,EXT4_PID_I,EXT4_PID_D,EXT4_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif
    } 
...
#ifdef TEMP_PID
        ,0,EXT5_PID_INTEGRAL_DRIVE_MAX,EXT5_PID_INTEGRAL_DRIVE_MIN,EXT5_PID_P,EXT5_PID_I,EXT5_PID_D,EXT5_PID_MAX,0,0,0,{
        0,0,0,0      }
#endif
    } 
...
TemperatureController heatedBedController = {
  NUM_EXTRUDER,HEATED_BED_SENSOR_TYPE,BED_SENSOR_INDEX,0,0,0,0,0,HEATED_BED_HEAT_MANAGER
#ifdef TEMP_PID
    ,0,HEATED_BED_PID_INTEGRAL_DRIVE_MAX,HEATED_BED_PID_INTEGRAL_DRIVE_MIN,HEATED_BED_PID_PGAIN,HEATED_BED_PID_IGAIN,HEATED_BED_PID_DGAIN,HEATED_BED_PID_MAX,0,0,0,{
    0,0,0,0  }
#endif
};
#else
...
void extruder_select(byte ext_num) {
...
#endif
  current_extruder->extrudePosition = printer_state.currentPositionSteps[ExtruderAxis];
  ...
  printer_state.currentPositionSteps[ExtruderAxis] = current_extruder->extrudePosition;
#endif
  printer_state.destinationSteps[ExtruderAxis] = printer_state.currentPositionSteps[ExtruderAxis];
  axis_steps_per_unit[ExtruderAxis] = current_extruder->stepsPerMM;
  inv_axis_steps_per_unit[3] = 1.0f/axis_steps_per_unit[ExtruderAxis];
  ...
  axis_travel_steps_per_sqr_second[3] = axis_steps_per_sqr_second[3] = max_acceleration_units_per_sq_second[3] * axis_steps_per_unit[ExtruderAxis];
  ...
  float fmax=((float)F_CPU/((float)printer_state.maxExtruderSpeed*TIMER0_PRESCALE*axis_steps_per_unit[ExtruderAxis]))*60.0; // Limit feedrate to interrupt speed
  ...
  if(dx || dy) {
    ...
    move_steps(dx,dy,0,0,homing_feedrate[XAxis],true,ALWAYS_CHECK_ENDSTOPS);
...
#endif
const short * const temptables[8] PROGMEM = {
  (short int *)&temptable_1[0][0],(short int *)&temptable_2[0][0],(short int *)&temptable_3[0][0],(short int *)&temptable_4[0][0]
#if NUM_TEMPS_USERTHERMISTOR0>0
...
const byte temptables_num[8] PROGMEM = {
  NUMTEMPS_1,NUMTEMPS_2,NUMTEMPS_3,NUMTEMPS_4,NUM_TEMPS_USERTHERMISTOR0,NUM_TEMPS_USERTHERMISTOR1,NUM_TEMPS_USERTHERMISTOR2,NUMTEMPS_8};
...
int read_raw_temperature(byte type,byte pin) {
  ...
  case 60: // AD8495 (Delivers 5mV/Â°C)
...
// .
// pid method calculates a constant system "gain" Ku.
// If 100% power yeilds +500C rise, the gain is 5, degrees per full power, or .05 degrees per 1% power.
// However, that depends on the rate the extruder is losing heat, which is not constant.
// push filament through at full speed, 100% power may yeild +200C, or 40% of the prior gain.
//
// Using gain, one can calulate power per degree, or 100/500 or 0.2% per degree for this example.
// The bias, or set point for 230c with 30C ambient would be (230-30)* 0.2 or 40%
// but if extruder were going full speed, the bias would be 100%.
//
// Ku appears to be power per degree per unit time. The time appears to be half the "cycle"
// which is in turn naturally about the lag time delaying power applied before temperature rises.
// This routine calculates the equivalent from the reponse to a step increase in power.
// A small step is used, so device deos not approach working temperatures.
// The time lag is a slightly fuzzy measure, but the power per degree is precise.
// Experimentally the results track close with the orignal tuning function.
// However, this function is not subject to failure to complete due to over voltage or timeout.
// This function is considerably faster. 
// Results are then converted to the PID constants with the same formulas as the original function.
//  

#ifdef TEMP_PID
const static long minuteM = 60000L;
const static long secondM = 1000L;
#define FLS(x) (((float)x)/1000.0)

// cohen-coon method from ZieglerÃ¢â‚¬â€œNichols method: http://en.wikipedia.org/wiki/Ziegler%E2%80%93Nichols_method
static void cohen_step_tune(TemperatureController *c, float *lagTime, float *lagRise, float *powerPerLagRise, byte isBed) {
  float currentTemp, coldTemp, maxTemp;
  unsigned long startTime, currentTime, lagEndTime;
  float bias = c->pidMax * 0.10; //10% power  we dont want an unsafe max temp, so test a small amount of heater power
  unsigned long changeTimeout =( isBed ? 240 : 30) *secondM; // Seconds to wait with no rise to identify steady state
  unsigned long steadyTimeout = (isBed ? 100 : 20) *minuteM; // Minutes to wait total, safety limit
  unsigned long printTime;
  float deltaTemp = (isBed ? 0.4 : 2); // picking 0.3% of "full scale" as a reasonable trigger point
  float lagMultiple = 10; 
  float lagCheck = 0;
  ...
  *powerPerLagRise=1;
  ...
  coldTemp = c->currentTemperatureC = conv_raw_temp(c->sensorType,c->currentTemperature); // no cal for autotune

  OUT_P("PID Autotune step response test");
  if (isBed) OUT_P_LN(" for heated bed");
  else OUT_P_LN(" for extruder");
  if (coldTemp>40.0) {
    OUT_P_LN("PID warning. Test should start with room temp heater.");
    // could wait here, but it isnt clear the user want to wait. Maybe just trying it out.
    // User can always wait for temps to cool and run again
  }
  // First, we need to determine time from power on to first temp rise.
  // After testing, about 0.4% of maximum range appears a good indicator.
  // maximum rangs is the temperature reachable if full power were applied and
  // no material limitations existed. This is about 500 degrees for hot end,
  // about 100 for heater bed.
  currentTemp = coldTemp;
  pwm_pos[c->pwmIndex] = bias;

  startTime = millis();
  printTime = startTime + 100;

  while(((currentTime=millis()) < startTime +  minuteM) && (currentTemp< coldTemp+deltaTemp)) { // if heat has not risen in a minute, it isnt gunna.
    if (currentTime >= printTime) {
      print_temperatures();
      printTime = currentTime + 1000;
      UI_MEDIUM; // minimize impact on measurement, 
      UI_SLOW;   // although it slows response for a few seconds, LCD still works
    }
    c->currentTemperature = read_raw_temperature(c->sensorType,c->sensorPin);
    currentTemp = c->currentTemperatureC = conv_raw_temp(c->sensorType,c->currentTemperature); // no cal for autotune
  }
  lagEndTime = millis();
  *lagTime = lagEndTime-startTime;

  if (currentTemp< coldTemp+deltaTemp) {
    // error condition, nothing heated up
    OUT_P_LN("PID error, no heating occured. Aborting test.");
    return;
  }
  coldTemp = currentTemp; // reset cold temp to start of ramp test
  OUT_P_F(" Start temperature is ",coldTemp);
  OUT_P_F_LN(",   Lag is ",FLS(*lagTime));
  lagEndTime = millis(); // remove IO time from measurement
  lagCheck = *lagTime; // print sample time/temp
  steadyTimeout = 1+ *lagTime * 5;
  maxTemp = currentTemp;

  for(currentTime=millis(); 
        (currentTime < lagEndTime+steadyTimeout); 
        currentTime=millis()) 
  {
    c->currentTemperature = read_raw_temperature(c->sensorType,c->sensorPin);
    currentTemp = c->currentTemperatureC = conv_raw_temp(c->sensorType,c->currentTemperature); // no cal for autotune
    ...
    if (currentTime >= lagEndTime+lagCheck) {
      OUT_P_F(" Temperature at time +",FLS(lagCheck));
      OUT_P_F(" = ",coldTemp);
      OUT_P_F_LN("+",currentTemp-coldTemp);
      lagCheck = lagCheck + *lagTime;
    }

    UI_MEDIUM;
    UI_SLOW;
  }
  *lagRise = (currentTemp-coldTemp)/5; // because we ran 5 lag checks.
  OUT_P_F(" Temperature max =",maxTemp);
  OUT_P_F_LN(" = +",maxTemp-coldTemp);
  *powerPerLagRise = bias/(*lagRise);
  OUT_P_F(" Test power= ",bias);
  OUT_P_F("   Lag temp rise= ",*lagRise);
  OUT_P_F_LN("   power per lag temp rise ",*powerPerLagRise);
}

void autotunePID(float temp, unsigned char heaterId, unsigned char methods) {
  TemperatureController *c;
  byte isBed = heaterId == NUM_EXTRUDER;
  if (heaterId >= NUM_TEMPERATURE_LOOPS) {
    OUT_P_LN("PID Autotune fatal error no such heater id.");
    return;
  }

  c = tempController[heaterId];
  if (methods & 2) {
    float gain, lagTime, lagRise;
    cohen_step_tune(c, &lagTime, &lagRise, &gain, isBed);
    // note gain = N/P from table 3.
    // from paper, Pk depends on timing, others depend on ratio of lag to total time
    //   float Pk = c->pidMax*timeRatio * (1.33 + timeRatio/4) /gain;
    //   float Ik = lagTime*(30+3*timeRatio)/(9+20*timeRatio);
    //   float Dk = 4* lagTime *(11+2*timeRatio);
    // Below in original autotune, Pk is simply 60% of raw gain.
    // other constants are multiples of total time
    float Tu = lagTime*2/1000; // make milliseconds into seconds for Tu of 2 lagTime's.
    // Tu is one lag up and one lag down.
    float Ku = gain*4/3.14; // formula matched to below. Dont know why 4/3.14?
    float Kp = 0.6*Ku;
    float Ki = 2*Kp/Tu;
    float Kd = Kp*Tu*0.125;
    OUT_P_F(" Step PID Ku: ", Ku);
    OUT_P_F_LN("    Tu: ", Tu);
    OUT_P_F_LN(" Kp: ",Kp);
    OUT_P_F_LN(" Ki: ",Ki);
    OUT_P_F_LN(" Kd: ",Kd);
...
// The following is to implement the closed loop version of p i d calculations from 
// ZieglerÃ¢â‚¬â€œNichols method: http://en.wikipedia.org/wiki/Ziegler%E2%80%93Nichols_method
// experiments show it may fail to produce usable results (timeout for example)

void autotunePID(float temp,unsigned char controllerId)
{
  TemperatureController *c = tempController[controllerId]; // This is not safe. Can produce memory access error for bad number
  float currentTemp;
  int cycles=0;
  bool heating = true;
  // to whom it may concern, i made modifications here to deal with the fact that
  // on my e3d head it would overshoot temp and fail
  // and on my heater bed with aluminum plate it would time out and fail.
  // By all means correct my work, it is a bit on the "guess"y side.
  bool isBed = controllerId == NUM_EXTRUDER;
  ...
  float startPwm = c->pidMax*(isBed ? 0.99 : 0.5); // avoid overshoot, but let bed heater go faster
  OUT_P_F_LN("PID Autotune start PWM= ",startPwm);
  pwm_pos[c->pwmIndex] = startPwm;
  ...
  for(;;) {
    ...
    if(heating == true && currentTemp > temp) { // switch heating -> off
      // bed id is == NUM_EXTRUDER , so below it is true for extruders, false for bed
      // so this test is using longer times for extruders than beds, which is confusing.
    ...
    if(heating == false && currentTemp < temp) {
      // bed id is == NUM_EXTRUDER , so below it is true for extruders, false for bed
      // so this test is using longer times for extruders than beds, which is confusing.
      if(time - t1 > (controllerId<NUM_EXTRUDER ? 5000 : 3000)) { // should be ? NUM_TEMPERATURE_LOOPS
        heating=true;
        t2=time;
        ...
        if(cycles > 0) {
          ...
          if(cycles > 2) {
            // Parameter according ZieglerÃ¢â‚¬â€œNichols method: http://en.wikipedia.org/wiki/Ziegler%E2%80%93Nichols_method
    ...
    if(currentTemp > (temp + 20.0)) {
      OUT_P_I(" heater: ",controllerId);
      OUT_P_F(" target: ",temp);
      OUT_P_F_LN(" current temp: ",currentTemp);
      OUT_P_LN("PID Autotune failed! Temperature too high");
      disableAllHeater();
      return;
    }
    ...
    if((time - (t1>t2? t1: t2)) > ((isBed ? 60:20) * minuteM)) { // stop doubling time difference, allow longer for beds
...
void manage_temperatures() {
  for(byte controller=0;controller<NUM_TEMPERATURE_LOOPS;controller++) {
    ...
    if (act->tempCalibration > UI_SET_MAX_TEMP_CAL || act->tempCalibration < UI_SET_MIN_TEMP_CAL 
      || isnan(act->tempCalibration)) act->tempCalibration=100.0;
    act->currentTemperatureC = (act->tempCalibration/100.0)*conv_raw_temp(act->sensorType,act->currentTemperature);
