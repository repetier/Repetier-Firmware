...
void printPosition() {
  OUT_P_F("X:",printer_state.currentPositionSteps[XAxis]*inv_axis_steps_per_unit[0]*(unit_inches?0.03937:1));
  OUT_P_F(" Y:",printer_state.currentPositionSteps[YAxis]*inv_axis_steps_per_unit[1]*(unit_inches?0.03937:1));
  OUT_P_F(" Z:",printer_state.currentPositionSteps[ZAxis]*inv_axis_steps_per_unit[2]*(unit_inches?0.03937:1));
  OUT_P_F_LN(" E:",printer_state.currentPositionSteps[ExtruderAxis]*inv_axis_steps_per_unit[3]*(unit_inches?0.03937:1));
}
...
void home_axis(bool xaxis,bool yaxis,bool zaxis) {
  ...
  if (X_MAX_PIN > -1 && Y_MAX_PIN > -1 && Z_MAX_PIN > -1 && MAX_HARDWARE_ENDSTOP_X & MAX_HARDWARE_ENDSTOP_Y && MAX_HARDWARE_ENDSTOP_Z) {
    ...
    if (homeallaxis || zaxis) {
      delta_move_to_top_endstops(homing_feedrate[XAxis]);	
      move_steps(0,0,axis_steps_per_unit[XAxis]*-ENDSTOP_Z_BACK_MOVE,0,homing_feedrate[XAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, false);
      delta_move_to_top_endstops(homing_feedrate[XAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR);	
      printer_state.currentPositionSteps[XAxis] = 0;
      printer_state.currentPositionSteps[YAxis] = 0;
      printer_state.currentPositionSteps[ZAxis] = printer_state.zMaxSteps;
      calculate_delta(printer_state.currentPositionSteps, printer_state.currentDeltaPositionSteps);
      printer_state.maxDeltaPositionSteps = printer_state.currentDeltaPositionSteps[0];
    } 
    else {
      if (xaxis) printer_state.destinationSteps[XAxis] = 0;
      if (yaxis) printer_state.destinationSteps[YAxis] = 0;
...
void home_axis(bool xaxis,bool yaxis,bool zaxis) {
  ...
  if(xaxis) {
    if ((MIN_HARDWARE_ENDSTOP_X && X_MIN_PIN > -1 && X_HOME_DIR==-1) || (MAX_HARDWARE_ENDSTOP_X && X_MAX_PIN > -1 && X_HOME_DIR==1)){
      ...
      printer_state.currentPositionSteps[XAxis] = -steps;
      move_steps(2*steps,0,0,0,homing_feedrate[XAxis],true,true);
      printer_state.currentPositionSteps[XAxis] = 0;
      move_steps(axis_steps_per_unit[XAxis]*-ENDSTOP_X_BACK_MOVE * X_HOME_DIR,0,0,0,homing_feedrate[XAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
      move_steps(axis_steps_per_unit[XAxis]*2*ENDSTOP_X_BACK_MOVE * X_HOME_DIR,0,0,0,homing_feedrate[XAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
#if defined(ENDSTOP_X_BACK_ON_HOME)
      if(ENDSTOP_X_BACK_ON_HOME > 0)
        move_steps(axis_steps_per_unit[XAxis]*-ENDSTOP_X_BACK_ON_HOME * X_HOME_DIR,0,0,0,homing_feedrate[XAxis],true,false);
#endif
      long offX = 0;
#if NUM_EXTRUDER>1
...
#endif
      printer_state.currentPositionSteps[XAxis] = (X_HOME_DIR == -1) ? printer_state.xMinSteps-offX : printer_state.xMaxSteps+offX;
#if NUM_EXTRUDER>1
      move_steps((current_extruder->xOffset-offX) * X_HOME_DIR,0,0,0,homing_feedrate[XAxis],true,false);      
#endif
    }
  }        
  if(yaxis) {
    if ((MIN_HARDWARE_ENDSTOP_Y && Y_MIN_PIN > -1 && Y_HOME_DIR==-1) || (MAX_HARDWARE_ENDSTOP_Y && Y_MAX_PIN > -1 && Y_HOME_DIR==1)){
      ...
      printer_state.currentPositionSteps[YAxis] = -steps;
      move_steps(0,2*steps,0,0,homing_feedrate[YAxis],true,true);
      printer_state.currentPositionSteps[YAxis] = 0;
      move_steps(0,axis_steps_per_unit[YAxis]*-ENDSTOP_Y_BACK_MOVE * Y_HOME_DIR,0,0,homing_feedrate[YAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
      move_steps(0,axis_steps_per_unit[YAxis]*2*ENDSTOP_Y_BACK_MOVE * Y_HOME_DIR,0,0,homing_feedrate[YAxis]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
#if defined(ENDSTOP_Y_BACK_ON_HOME)
      if(ENDSTOP_Y_BACK_ON_HOME > 0)
        move_steps(0,axis_steps_per_unit[YAxis]*-ENDSTOP_Y_BACK_ON_HOME * Y_HOME_DIR,0,0,homing_feedrate[YAxis],true,false);
#endif
      long offY = 0;
#if NUM_EXTRUDER>1
      ...
      // Reposition extruder that way, that all extruders can be selected at home pos.
#endif
      printer_state.currentPositionSteps[YAxis] = (Y_HOME_DIR == -1) ? printer_state.yMinSteps-offY : printer_state.yMaxSteps+offY;
#if NUM_EXTRUDER>1
      move_steps(0,(current_extruder->yOffset-offY) * Y_HOME_DIR,0,0,homing_feedrate[YAxis],true,false);      
#endif
    }
  }        
  if(zaxis) {
    if ((MIN_HARDWARE_ENDSTOP_Z && Z_MIN_PIN > -1 && Z_HOME_DIR==-1) || (MAX_HARDWARE_ENDSTOP_Z && Z_MAX_PIN > -1 && Z_HOME_DIR==1)){
      ...
      printer_state.currentPositionSteps[ZAxis] = -steps;
      move_steps(0,0,2*steps,0,homing_feedrate[ZAxis],true,true);
      printer_state.currentPositionSteps[ZAxis] = 0;
      move_steps(0,0,axis_steps_per_unit[ZAxis]*-ENDSTOP_Z_BACK_MOVE * Z_HOME_DIR,0,homing_feedrate[ZAxis]/ENDSTOP_Z_RETEST_REDUCTION_FACTOR,true,false);
      move_steps(0,0,axis_steps_per_unit[ZAxis]*2*ENDSTOP_Z_BACK_MOVE * Z_HOME_DIR,0,homing_feedrate[ZAxis]/ENDSTOP_Z_RETEST_REDUCTION_FACTOR,true,true);
#if defined(ENDSTOP_Z_BACK_ON_HOME)
      if(ENDSTOP_Z_BACK_ON_HOME > 0)
        move_steps(0,0,axis_steps_per_unit[ZAxis]*-ENDSTOP_Z_BACK_ON_HOME * Z_HOME_DIR,0,homing_feedrate[ZAxis],true,false);
#endif
      printer_state.currentPositionSteps[ZAxis] = (Z_HOME_DIR == -1) ? printer_state.zMinSteps : printer_state.zMaxSteps;
    }
  }
  UI_CLEAR_STATUS  
}
...
{
  register long Xp; // GCode X converted to steps
  register long Yp; // GCode Y converted to steps
  register long Zp; // GCode Z converted to steps

  register byte r= false;
  float X; // Absoluts X coordinates in millimeters
  float Y; // Absoluts Y coordinates in millimeters
  float Z; // Absoluts Z coordinates in millimeters
  byte inBounds = true;

  // BOUNDS CHECK!
  // Avoid wrecking hardware by out of bound moves!
  // First, get Absolute X, Y and Z in millimeters
  // and absolte Xp, Yp and Zp in steps
  // All X, Y and Z steps is calculated as before this safety bounds check improvement.

  if(GCODE_HAS_X(com)) {
    r = true;
    X = com->X;
    if(unit_inches) X = X*25.4;
    Xp = X*axis_steps_per_unit[XAxis];
    if(relative_mode) 
      Xp = Xp + printer_state.currentPositionSteps[XAxis];
    else 
      Xp = Xp+printer_state.offsetX;
  } 
  else Xp = printer_state.currentPositionSteps[XAxis];
  X = Xp*inv_axis_steps_per_unit[XAxis];

  if(GCODE_HAS_Y(com)) {
    r = true;
    Y = com->Y;
    if(unit_inches) Y = Y*25.4;
    Yp = Y*axis_steps_per_unit[YAxis];
    if(relative_mode) 
      Yp = Yp + printer_state.currentPositionSteps[YAxis];
    else 
      Yp = Yp+printer_state.offsetY;
  } 
  else Yp = printer_state.currentPositionSteps[YAxis];
  Y = Yp*inv_axis_steps_per_unit[YAxis];

  if(GCODE_HAS_Z(com)) {
    r = true;
    Z = com->Z;
    if(unit_inches) Z = Z*25.4;
    Zp = Z*axis_steps_per_unit[ZAxis];
    if(relative_mode) 
      Zp = Zp + printer_state.currentPositionSteps[ZAxis];
    else 
      Zp = Zp;
  } 
  else Zp = printer_state.currentPositionSteps[ZAxis];
  Z = Zp*inv_axis_steps_per_unit[ZAxis];

  // Now check bounds
  // Z bounds check same for delta or cartesian drive system.
  if ((Z < printer_state.zMin) || (Z > printer_state.zLength)) inBounds = false;
#if DRIVE_SYSTEM==DeltaDrive
  // For delta printers, print area is round. Delta XY Radius is the safe print limit.
  if ( ((X*X) + (Y*Y)) > (printer_state.delta_xy_radius * printer_state.delta_xy_radius) ) inBounds = false;
#else
  // for cartesian printers, X-Y are rectangular. Simple min/max test.
  if ((X < printer_state.xMin) || (X > printer_state.xLength)) inBounds = false;
  if ((Y < printer_state.yMin) || (Y > printer_state.yLength)) inBounds = false;
#endif

  if ( !inBounds ) {
    // add com->N to output!
    reportOutBoundsLine(com);
    return false; // nulifies command movement, avoids damaging printer.
  }

  //If we reach here, bounds check has passed. It is ok to update printer state.
  printer_state.destinationSteps[XAxis] = Xp;
  printer_state.destinationSteps[YAxis] = Yp;
  printer_state.destinationSteps[ZAxis] = Zp;

  if(GCODE_HAS_E(com) && !DEBUG_DRYRUN) {
    register long p;
    if(unit_inches)
      p = com->E*25.4*axis_steps_per_unit[ExtruderAxis];
    else
      p = com->E*axis_steps_per_unit[ExtruderAxis];
    if(relative_mode || relative_mode_e)
      printer_state.destinationSteps[ExtruderAxis] = printer_state.currentPositionSteps[ExtruderAxis]+p;
    else
      printer_state.destinationSteps[ExtruderAxis] = p;
    if (p != printer_state.currentPositionSteps[ExtruderAxis]) // ignore unproductive moves
      r = true;
  } 
  else printer_state.destinationSteps[ExtruderAxis] = printer_state.currentPositionSteps[ExtruderAxis];
  if(GCODE_HAS_F(com)) {
    if(com->F < 1)
  ...
  return r; // productivity test moved to ExtruderAxis above for efficiency
}
...
void process_command(GCode *com)
{
  ...
  // If commands are being sent, any commends, do not time out
  // if repetier host is connected, the printer will stay on to report temp
  // process files to/from SD, etc. calling the function indicates
  // the printer is being asked to do useful work, and should not time out.
  // Does not time out during debug communication
  // Previously, many code paths failed to indicate activity
  // being selective about what is an "activity" misses the fact that
  // any activity indicates ongoing use by file or computer, which may do printing next.
  previous_millis_cmd = millis(); // Moved activity indicator here

#ifdef SDSUPPORT
  if(sd.savetosd){
    if(GCODE_HAS_M(com) && com->M==29) { 
      // M29 means done writing to file.
      // Fall thru to normal command  processing to close sd file
    } 
    else {
      sd.write_command(com);
    }
  }
#endif

  // monitor of code in buffer, but skip the m105's since they are continuos from host
  if (!GCODE_HAS_M(com) || com->M != 105) reportLastLine(com); 

  if (GCODE_HAS_G(com) && GCODE_HAS_M(com)) {
    reportBadLine(com,PSTR(" has conflicting commands"));
  } else
  if(GCODE_HAS_G(com))
  {
    switch(com->G)
    ...
    case 3: // CCW Arc MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
      {
        ...
        float offset[2] = {
          (GCODE_HAS_I(com)?com->I:0),(GCODE_HAS_J(com)?com->J:0)        };
        if(unit_inches) {
          offset[0]*=25.4;
          offset[1]*=25.4;
        }
        float position[2] = {
          printer_state.currentPositionSteps[XAxis]*inv_axis_steps_per_unit[0],printer_state.currentPositionSteps[YAxis]*inv_axis_steps_per_unit[1]        };
        float target[2] = {
          printer_state.destinationSteps[XAxis]*inv_axis_steps_per_unit[0],printer_state.destinationSteps[YAxis]*inv_axis_steps_per_unit[1]        };    
        float r;
        if (GCODE_HAS_R(com)) {
          /* 
    ...
    case 91: // G91
      ...
      break;
    case 92: // G92
      if(GCODE_HAS_X(com)) printer_state.currentPositionSteps[XAxis] = com->X*axis_steps_per_unit[XAxis]*(unit_inches?25.4:1.0)-printer_state.offsetX;
      if(GCODE_HAS_Y(com)) printer_state.currentPositionSteps[YAxis] = com->Y*axis_steps_per_unit[YAxis]*(unit_inches?25.4:1.0)-printer_state.offsetY;
      if(GCODE_HAS_Z(com)) printer_state.currentPositionSteps[ZAxis] = com->Z*axis_steps_per_unit[ZAxis]*(unit_inches?25.4:1.0);
      if(GCODE_HAS_E(com)) {
        printer_state.currentPositionSteps[ExtruderAxis] = com->E*axis_steps_per_unit[ExtruderAxis]*(unit_inches?25.4:1.0);
      }
      break;
    default: 
      reportBadLine(com, PSTR(" has undefined command code"));
    }
  }
    ...
    case 109: // M109 - Wait for extruder heater to reach target.
      {
        ...
        do {
          ...
          if (actExtruder==current_extruder && actExtruder->waitRetractUnits > 0 && !retracted && dir && actExtruder->tempControl.currentTemperatureC > actExtruder->waitRetractTemperature) {
            move_steps(0,0,0,-actExtruder->waitRetractUnits * axis_steps_per_unit[ExtruderAxis],actExtruder->maxFeedrate,false,false);
            retracted = 1;
          }
#endif
    ...
    case 303: 
      {
        ...
        unsigned char methods = 1; // old method
        float p = 0;
        if(GCODE_HAS_S(com)) temp = com->S;
        if(GCODE_HAS_P(com)) p = com->P; // 1 2 or 1+2 (3)
        if(GCODE_HAS_T(com)) { // T is preferred method for selecting extruder
          cont = com->T;
          methods = p+.1; //make sure rounds off as expected to integer.
          if (methods ==0) methods =2;
        } 
        else {
          cont = p; // backwards compatible
        }
        if(cont>=NUM_TEMPERATURE_LOOPS) cont = NUM_TEMPERATURE_LOOPS-1; // correct to valid index
        if(cont == NUM_TEMPERATURE_LOOPS-1) {
          // bed heater.
          // do NOT assume it means 150C by default.
          if(!GCODE_HAS_S(com)) temp = 70.0;
        }
        autotunePID(temp, cont, methods);
      }
      break;
#endif
    ...
    case 85: // M85
      ...
      break;
    case 92: // M92
      if(GCODE_HAS_X(com)) axis_steps_per_unit[XAxis] = com->X;
      if(GCODE_HAS_Y(com)) axis_steps_per_unit[YAxis] = com->Y;
      if(GCODE_HAS_Z(com)) axis_steps_per_unit[ZAxis] = com->Z;
    ...
    case 117: // M117 message to lcd
      if(GCODE_HAS_STRING(com)) {
        UI_USER(com->text); // wont get clobbered by system status
    ...
    case 201: // M201
      if(GCODE_HAS_X(com)) max_acceleration_units_per_sq_second[XAxis] = com->X;
      if(GCODE_HAS_Y(com)) max_acceleration_units_per_sq_second[YAxis] = com->Y;
      if(GCODE_HAS_Z(com)) max_acceleration_units_per_sq_second[ZAxis] = com->Z;
      if(GCODE_HAS_E(com)) max_acceleration_units_per_sq_second[3] = com->E;
      update_ramps_parameter();
      break;
    case 202: // M202
      if(GCODE_HAS_X(com)) max_travel_acceleration_units_per_sq_second[XAxis] = com->X;
      if(GCODE_HAS_Y(com)) max_travel_acceleration_units_per_sq_second[YAxis] = com->Y;
      if(GCODE_HAS_Z(com)) max_travel_acceleration_units_per_sq_second[ZAxis] = com->Z;
    ...
    case 401: // Memory position
      printer_state.memoryX = printer_state.currentPositionSteps[XAxis];
      printer_state.memoryY = printer_state.currentPositionSteps[YAxis];
      printer_state.memoryZ = printer_state.currentPositionSteps[ZAxis];
      break;
    case 402: // Go to stored position
      {
        bool all = !(GCODE_HAS_X(com) && GCODE_HAS_Y(com) && GCODE_HAS_Z(com));
        move_steps((all || GCODE_HAS_X(com) ? printer_state.memoryX-printer_state.currentPositionSteps[XAxis] : 0)
          ,(all || GCODE_HAS_Y(com) ? printer_state.memoryY-printer_state.currentPositionSteps[YAxis] : 0)
          ,(all || GCODE_HAS_Z(com) ? printer_state.memoryZ-printer_state.currentPositionSteps[ZAxis] : 0)
...
#endif
    default: 
      reportBadLine(com, PSTR(" has undefined M command code"));
    }// end of M switch statement

  } 
  else if(GCODE_HAS_T(com))  { // Process T code
    wait_until_end_of_move();
    extruder_select(com->T);
    if (com->T >= NUM_EXTRUDER) // cant select heater bed
      reportBadLine(com, PSTR(" command selects undefined extruder."));
  } 
  else{
    reportBadLine(com, PSTR(" Unknown command."));
  }
}
