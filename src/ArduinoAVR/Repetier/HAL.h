...
#if MOTHERBOARD == 8 || MOTHERBOARD == 88 || MOTHERBOARD == 9 ||               \
    MOTHERBOARD == 92 || CPU_ARCH != ARCH_AVR
...
// Can not change buffer size here, need add build flag -D
...
public:
  inline void protect() { cli(); }

  inline void unprotect() { SREG = sreg; }
  ...
  inline ~InterruptProtectedBlock() { SREG = sreg; }
};
...
// leave some margin for all the extra transformations. So we keep inside clean
// timings.
...
public:
  ...
  Normally the C compiler will compute a long/long division, which takes ~670
  Ticks. This version is optimized for a 16 bit dividend and recognizes the
  special cases of a 24 bit and 16 bit dividend, which often, but not always
  occur in updating the interval.
  */
  static inline int32_t Div4U2U(uint32_t a, uint16_t b) {
#if CPU_ARCH == ARCH_AVR
    // r14/r15 remainder
    // r16 counter
    __asm__ __volatile__("clr r14 \n\t"
                         "sub r15,r15 \n\t"
                         "tst %D0 \n\t"
                         "brne do32%= \n\t"
                         "tst %C0 \n\t"
  ...
  static inline unsigned int ComputeV(long timer, long accel) {
    ...
    unsigned int res;
    // 38 Ticks
    __asm__ __volatile__( // 0 = res, 1 = timer, 2 = accel %D2=0 ,%A1 are unused
                          // is free Result LSB first: %A0, %B0, %A1
        "mul %B1,%A2 \n\t"
        "mov %A0,r1 \n\t"
        "mul %B1,%C2 \n\t"
        "mov %B0,r0 \n\t"
  ...
  static inline void tone(uint8_t pin, int duration) { ::tone(pin, duration); }
  static inline void noTone(uint8_t pin) { ::noTone(pin); }
  ...
  static inline float eprGetFloat(unsigned int pos) {
    float v;
    eeprom_read_block(&v, (void *)(EEPROM_OFFSET + pos),
                      4); // newer gcc have eeprom_read_block but not arduino 22
    return v;
  }
  ...
  // interrupt handler.
  static inline void allowInterrupts() { sei(); }
  ...
  static inline void serialSetBaudrate(long baud) { RFSERIAL.begin(baud); }
  static inline bool serialByteAvailable() { return RFSERIAL.available() > 0; }
  static inline uint8_t serialReadByte() { return RFSERIAL.read(); }
  static inline void serialWriteByte(char b) { RFSERIAL.write(b); }
  static inline void serialFlush() { RFSERIAL.flush(); }
