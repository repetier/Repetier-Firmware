...
{
    int32_t z=cartesianPosSteps[Z_AXIS];
#if DISTORTION_CORRECTION
    z+=Printer::distortion.correct(cartesianPosSteps[X_AXIS], cartesianPosSteps[Y_AXIS]);
#endif
    ...
    if(Printer::isLargeMachine())
    {
     ...
     // A TOWER height
        ...
        deltaPosSteps[A_TOWER] = HAL::integer64Sqrt(opt-temp) + z;
        ...
        deltaPosSteps[B_TOWER] = HAL::integer64Sqrt(opt-temp) + z ;
        ...
        deltaPosSteps[C_TOWER] = HAL::integer64Sqrt(opt - temp) + z;
        ...
        if ((temp = opt - temp2 * temp2) >= 0)
            deltaPosSteps[A_TOWER] = floor(0.5 + sqrt(temp)) + z;
        else
            return 0;
        ...
        if ((temp = opt - temp2 * temp2) >= 0)
            deltaPosSteps[B_TOWER] = floor(0.5 + sqrt(temp)) + z;
        else
            return 0;
        ...
        if ((temp = opt - temp2*temp2) >= 0)
            deltaPosSteps[C_TOWER] = floor(0.5 + sqrt(temp)) + z;
        else
            return 0;
    ...
    else
    {
        ...
        deltaPosSteps[A_TOWER] = SQRT(opt-temp) + z;
        if (deltaPosSteps[A_TOWER] < Printer::deltaFloorSafetyMarginSteps && !Printer::isZProbingActive())
            RETURN_0("A hit floor");
        ...
        deltaPosSteps[B_TOWER] = SQRT(opt-temp) + z ;
        if (deltaPosSteps[B_TOWER] < Printer::deltaFloorSafetyMarginSteps && !Printer::isZProbingActive())
            RETURN_0("B hit floor");
        ...
        deltaPosSteps[C_TOWER] = SQRT(opt - temp) + z;
        if (deltaPosSteps[C_TOWER] < Printer::deltaFloorSafetyMarginSteps && !Printer::isZProbingActive())
            RETURN_0("C hit floor");
        /*
