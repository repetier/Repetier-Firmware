...
secondspeed_t LaserDriver::intensity =
    LASER_PWM_MAX; // Intensity to use for next move queued if we want lasers.
...
void CNCDriver::spindleOnCW(int32_t rpm) {
  spindleSpeed =
      map(rpm, 0, CNC_RPM_MAX, 0, CNC_PWM_MAX); // linear interpolation
