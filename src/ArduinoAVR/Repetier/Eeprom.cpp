...
//#define EPR_ACCELERATION_TYPE 1
    ...
    if (corrupted) {
        ...
        EVENT_EEPROM_STORE_DATA_INTO_EEPROM
        initalizeUncached();
    }
...
void EEPROM::initalizeUncached() {
    ...
    EVENT_EEPROM_INIT_UNCACHED
}

void EEPROM::readDataFromEEPROM(bool includeExtruder) {
#if EEPROM_MODE != 0
    uint8_t version = HAL::eprGetByte(EPR_VERSION); // This is the saved version. Don't copy
...
void EEPROM::updatePrinterUsage() {
    ...
    HAL::eprSetFloat(EPR_PRINTING_DISTANCE,
                     HAL::eprGetFloat(EPR_PRINTING_DISTANCE) + Printer::filamentPrinted * 0.001);
...
void EEPROM::storeMixingRatios(bool _updateChecksum) {
    for (uint8_t e = 0; e < NUM_EXTRUDER; e++) {
        for (uint8_t i = 0; i < VIRTUAL_EXTRUDER; i++) {
            HAL::eprSetInt16(EEPROM_EXTRUDER_OFFSET + e * EEPROM_EXTRUDER_LENGTH + EPR_EXTRUDER_MIXING_RATIOS + 2 * i,
