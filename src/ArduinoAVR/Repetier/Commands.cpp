...
const int8_t sensitive_pins[] PROGMEM =
    SENSITIVE_PINS; // Sensitive pin list for M42
...
void Commands::printCurrentPosition() {
  ...
  Com::printFLN(Com::tSpaceEColon,
                Printer::currentPositionSteps[E_AXIS] *
                    ...
                    (Printer::unitIsInches ? 0.03937 : 1),
                4);
#ifdef DEBUG_POS
...
void Commands::printTemperatures(bool showRaw) {
  ...
  if (showRaw) {
    ...
    Com::printF(Com::tColon, (1023 << (2 - ANALOG_REDUCE_BITS)) -
                                 heatedBedController.currentTemperature);
  }
  ...
  for (uint8_t i = 0; i < NUM_EXTRUDER; i++) {
    ...
    if (showRaw) {
      ...
      Com::printF(Com::tColon, (1023 << (2 - ANALOG_REDUCE_BITS)) -
                                   extruder[i].tempControl.currentTemperature);
    }
  }
  ...
  if (showRaw) {
    ...
    Com::printF(Com::tColon, (1023 << (2 - ANALOG_REDUCE_BITS)) -
                                 extruder[0].tempControl.currentTemperature);
  }
#endif
#ifdef FAKE_CHAMBER
...
void Commands::changeFlowrateMultiply(int factor) {
  ...
  if (Extruder::current->diameter <= 0)
    ...
    Printer::extrusionFactor = 0.01f * static_cast<float>(factor) * 4.0f /
                               (Extruder::current->diameter *
                                Extruder::current->diameter * 3.141592654f);
  ...
  if (PrintLine::linesCount == 0 || immediately) {
    if (Printer::mode == PRINTER_MODE_FFF) {
      for (fast8_t i = 0; i < PRINTLINE_CACHE_SIZE; i++)
        PrintLine::lines[i].secondSpeed =
            speed; // fill all printline buffers with new fan speed value
    }
...
void dacAnalogUpdate(bool saveEEPROM = false) {
  ...
  for (int i = 0; i < MCP4728_NUM_CHANNELS; i++) {
    ...
    uint8_t highbyte =
        (_intVref[i] << 7 | _gain[i] << 4 | (uint8_t)((level) >> 8));
...
void dacPrintSet(int dacChannelSettings[], const char *dacChannelPrefixes[]) {
  for (int i = 0; i < MCP4728_NUM_CHANNELS; i++) {
    uint8_t dac_channel =
        dac_stepper_channel[i]; // DAC Channel is a mapped lookup.
    Com::printF(dacChannelPrefixes[i], ((float)dacChannelSettings[dac_channel] *
                                        100 / MCP4728_VOUT_MAX));
...
void setMotorCurrent(uint8_t xyz_channel, uint16_t level) {
  ...
  dac_motor_current[stepper_channel] =
      level < MCP4728_VOUT_MAX ? level : MCP4728_VOUT_MAX;
  dacAnalogUpdate();
}
...
void Commands::processArc(GCode *com) {
  ...
  if (com->hasR()) {
    ...
    */
    ...
    double h_x2_div_d =
        -sqrt(4 * r * r - x * x - y * y) / hypot(x, y); // == -(h * 2 / d)
...
void Commands::processGCode(GCode *com) {
...
#if defined(SUPPORT_LASER) && SUPPORT_LASER
  {
    bool laserOn = LaserDriver::laserOn;
#if LASER_WARMUP_TIME > 0
    if (Printer::mode == PRINTER_MODE_LASER && LaserDriver::firstMove &&
        (LaserDriver::laserOn || com->hasE())) {
      PrintLine::waitForXFreeLines(1, true);
      PrintLine::LaserWarmUp(LASER_WARMUP_TIME);
      LaserDriver::firstMove = false;
  ...
  case 4: // G4 dwell
    ...
    codenum +=
        HAL::timeInMilliseconds(); // keep track of when we started waiting
    ...
    if (ok) {
      ...
      if (com->hasS() && com->S) {
        ...
        Printer::currentPositionSteps[Z_AXIS] =
            sum * Printer::axisStepsPerMM[Z_AXIS];
  ...
  case 30: {
    ...
    if (com->hasS()) {
      ...
      float z =
          Printer::runZProbe(p & 1, p & 2, Z_PROBE_REPETITIONS, true, false);
      ...
      if (com->hasR() || com->hasH()) {
        ...
        if (Printer::distortion.isEnabled()) {
          zCorr =
              Printer::distortion.correct(Printer::currentPositionSteps[X_AXIS],
        ...
        Printer::currentPosition[Z_AXIS] = Printer::lastCmdPos[Z_AXIS] =
            z + h + Printer::zMin;
  ...
  case 33: {
    ...
    } else if (com->hasX() || com->hasY() ||
               com->hasZ()) { // G33 X<xpos> Y<ypos> Z<zCorrection> - Set
    ...
    if (com->hasE()) {
      Printer::destinationPositionTransformed[E_AXIS] =
          Printer::currentPositionTransformed[E_AXIS] =
              Printer::convertToMM(com->E);
      Printer::destinationSteps[E_AXIS] =
          Printer::currentPositionSteps[E_AXIS] =
              Printer::destinationPositionTransformed[E_AXIS] *
      ...
      if (com->hasX() || com->hasY() || com->hasZ())
        ...
        if (Printer::currentPosition[X_AXIS] == 0 &&
          ...
          if (Printer::isLargeMachine()) {
            ...
            unsigned long bSteps =
                Printer::currentNonlinearPositionSteps[B_TOWER];
...
void Commands::processMCode(GCode *com) {
  ...
  case 104: // M104 temperature
    ...
    if (com->hasS()) {
      ...
      if (com->hasT() && com->T < NUM_EXTRUDER)
        Extruder::setTemperatureForExtruder(
            extruder[com->T].tempControl.preheatTemperature +
                (com->hasO() ? com->O : 0),
            com->T, com->hasF() && com->F > 0);
      else
        Extruder::setTemperatureForExtruder(
            Extruder::current->tempControl.preheatTemperature +
                (com->hasO() ? com->O : 0),
...
#if NUM_EXTRUDER > 0
  {
    ...
    if (com->hasS())
      ...
      Extruder::setTemperatureForExtruder(
          actExtruder->tempControl.preheatTemperature +
              (com->hasO() ? com->O : 0),
  ...
  case 190: { // M190 - Wait bed for heater to reach target.
    ...
    if (abs(heatedBedController.currentTemperatureC -
            heatedBedController.targetTemperatureC) < SKIP_M190_IF_WITHIN)
      break;
#endif
...
#if HAVE_HEATED_BED
      if (com->hasB()) {
        mod |= heatedBedController.preheatTemperature !=
               static_cast<int16_t>(com->B);
        heatedBedController.preheatTemperature = com->B;
      }
#endif
#if NUM_EXTRUDER > 0
      if (com->hasT() && com->hasS() && com->T < NUM_EXTRUDER) {
        mod |= extruder[com->T].tempControl.preheatTemperature !=
               static_cast<int16_t>(com->S);
  ...
  case 280: // M280
    ...
    if (com->hasS() && com->S > 0) {
      ...
      PrintLine::moveRelativeDistanceInSteps(
          -Extruder::current->xOffset +
              static_cast<int32_t>(Printer::xLength * 0.5 *
  ...
  case 290: // M290 Z<babysteps> - Correct by adding baby steps for Z mm
    ...
    if (com->hasZ()) {
      if (abs(com->Z) < (32700 - labs(Printer::zBabystepsMissing)) *
  ...
  case 302: // M302 S<0 or 1> - allow cold extrusion. Without S parameter it
    ...
    Printer::setColdExtrusionAllowed(!com->hasS() ||
                                     (com->hasS() && com->S != 0));
    break;
  case 303: { // M303
#if NUM_TEMPERATURE_LOOPS > 0
