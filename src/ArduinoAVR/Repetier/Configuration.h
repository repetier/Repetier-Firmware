...
when transferring from older configs!
...
This means, after the first upload many variables can only be changed using the
special M commands as described in the documentation. Changing these values in
the configuration.h file has no effect. Parameters overridden by EEPROM settings
are calibration values, extruder values except thermistor tables and some other
parameter likely to change during usage like advance steps or ops mode. To
override EEPROM settings with config settings, set EEPROM_MODE 0

*/

// BASIC SETTINGS: select your board type, thermistor type, axis scaling, and
...
// Melzi board                = 63  // Define REPRAPPRO_HUXLEY if you have one
...
// Go into fatal error mode when endstop is not triggered during homing
...
//in. Requires more ram, has only 63 byte input buffer.

/*
We can connect BlueTooth to serial converter module directly to boards based on
AtMega2560 or AtMega1280 and some boards based on AtMega2561, AtMega1281 or
AtMega1284p
- On Melzi boards connect BT to TX1 and RX1 pins, then set BLUETOOTH_SERIAL to 1
- On RUMBA boards connect BT to pin 11 and 12 of X3 connector, then set
BLUETOOTH_SERIAL to 3
- On RAMBO boards connect BT to pins 5,6 or 7,8 or 9,10 on Serial connector,
then accordingly set BLUETOOTH_SERIAL to 1,2 or 3
- On RAMPS we must remap Y_ENDSTOPS pins or Z_ENDSTOPZ pins or LCD_ENABLE and
...
is a full cartesian system where x, y and z moves are handled by separate
motors.
...
Cases 1, 2, 8 and 9 cover all needed xy and xz H gantry systems. If you get
results mirrored etc. you can swap motor connections for x and y. If a motor
turns in the wrong direction change INVERT_X_DIR or INVERT_Y_DIR.
*/
#define DRIVE_SYSTEM 0
/*
  Normal core xy implementation needs 2 virtual steps for a motor step to
  guarantee that every tiny move gets maximum one step regardless of direction.
  This can cost some speed, so alternatively you can activate the FAST_COREXYZ
  by uncommenting the define. This solves the core movements as nonlinear
  movements like done for deltas but without the complicated transformations.
  Since transformations are still linear you can reduce delta computations per
  second to 10 and also use 10 subsegments instead of 20 to reduce memory usage.
*/
//#define FAST_COREXYZ
...
/** \brief Number of steps for a 1mm move in z direction  Overridden if EEPROM
 * activated.*/
...
// These commands get executed after storing position and going to park
// position.
...
// extruder, not out.  Overridden if EEPROM activated.
...
// 60 is AD8494, AD8495, AD8496 or AD8497 (5mV/degC and 1/4 the price of AD595
// but only MSOT_08 package) 61 is AD8494, AD8495, AD8496 or AD8497 (5mV/degC
// and 1.25 Vref offset like adafruit breakout) 97 Generic thermistor table 1 98
// Generic thermistor table 2 99 Generic thermistor table 3 100 is AD595 101 is
// MAX6675 102 is MAX31855 103 is MAX31855 with software SPI, sensor pin is data
// input!
...
- 1 = PID Temperature control. Is better but needs good PID values. Defaults are
a good start for most extruder.
...
temperature. To prevent a long deviation from the target zone, this value limits
the lower value. A good start is 30 lower then the optimal value. You need to
leave room for cooling. Overridden if EEPROM activated.
*/
...
// maximum time the heater is can be switched on. Max = 255.  Overridden if
// EEPROM activated.
#define EXT0_PID_MAX 255
/** \brief Faktor for the advance algorithm. 0 disables the algorithm.
Overridden if EEPROM activated. K is the factor for the quadratic term, which is
normally disabled in newer versions. If you want to use the quadratic factor
make sure ENABLE_QUADRATIC_ADVANCE is defined. L is the linear factor and seems
to be working better then the quadratic dependency.
*/
...
/** \brief Units (mm/inches) to retract filament when extruder is heating up.
Overridden if EEPROM activated. Set to 0 to disable.
*/
#define EXT0_WAIT_RETRACT_UNITS 0

/** You can run any GCODE command on extruder deselect/select. Separate multiple
commands with a new line \n. That way you can execute some mechanical components
needed for extruder selection or retract filament or whatever you need. The
codes are only executed for multiple extruder when changing the extruder. */
...
/** Pin which toggles regularly during extrusion allowing jam control. -1 =
 * disabled */
...
- 0 = Simply switch on/off if temperature is reached. Works always.
- 1 = PID Temperature control. Is better but needs good PID values. Defaults are
a good start for most extruder. Overridden if EEPROM activated.
*/
...
/** The extruder cooler is a fan to cool the extruder when it is heating. If you
 * turn the etxruder on, the fan goes on. */
...
It will change the current extruders filament and temperature must already be
high enough.
*/
...
/** Will first retract short distance, go to change position and then retract
longretract. Retractions speeds are taken from RETRACTION_SPEED and
...
If you get much overshoot at the first temperature set, because the heater is
going full power too long, you need to increase this value. For one 6.8 Ohm
...
EXT0_PID_MAX by to methods. Set the value to 0: Normal computation, just clip
output to EXT0_PID_MAX if computed value is too high. Set value to 1: Scale PID
by EXT0_PID_MAX/256 and then clip to EXT0_PID_MAX. If your EXT0_PID_MAX is low,
you should prefer the second method.
*/
...
There are many different thermistors, which can be combined with different
resistors. This result in unpredictable number of tables. As a resolution, the
user can define one table here, that can be used as type 5 for thermistor type
in extruder/heated bed definition. Make sure, the number of entries matches the
value in NUM_TEMPS_USERTHERMISTOR0. If you span definition over multiple lines,
make sure to end each line, except the last, with a backslash. The table format
is {{adc1,temp1},{adc2,temp2}...} with increasing adc values. For more
informations, read
http://hydraraptor.blogspot.com/2007/10/measuring-temperature-easy-way.html

If you have a sprinter temperature table, you have to multiply the first value
with 4 and the second with 8. This firmware works with increased precision, so
the value reads go from 0 to 4095 and the temperature is temperature*8.

If you have a PTC thermistor instead of a NTC thermistor, keep the adc values
increasing and use thermistor types 50-52 instead of 5-7!
*/
...
If you don't feel like computing the table on your own, you can use this generic
method. It is a simple approximation which may be not as accurate as a good
table computed from the reference values in the datasheet. You can increase
precision if you use a temperature/resistance for R0/T0, which is near your
operating temperature. This will reduce precision for lower temperatures, which
are not really important. The resistors must fit the following schematic:
@code
...
The capacitor is for reducing noise from long thermistor cable. If you don't
have one, it's OK.
...
/** Supply voltage to ADC, can be changed by setting ANALOG_REF below to
 * different value. */
#define GENERIC_THERM_VREF 5
/** Number of entries in generated table. One entry takes 4 bytes. Higher number
of entries increase computation time too. Value is used for all generic tables
created. */
...
// If you use software spi for max31855 all chips need to use same CS/CLK pin.
// Sensor pin is MISO pin!
//#define SUPPORT_MAX31855_SW
#define MAX31855_SW_CS 47
#define MAX31855_SW_CLK 32

/** WIth value 1 you can adjust measured temperature into temp = measured * gain
 * + bias. */
#define TEMP_GAIN 0

// ############# Heated bed configuration ########################

/** \brief Set true if you have a heated bed connected to your board, false if
 * not */
#define HAVE_HEATED_BED 1

#define HEATED_BED_MAX_TEMP 115
/** Skip M190 wait, if heated bed is already within x degrees. Fixed numbers
 * only, 0 = off. */
...
1 = PID controlled
2 = Bang Bang, limited check every HEATED_BED_SET_INTERVAL. Use this with
relay-driven beds to save life time 3 = dead time control
*/
...
// Time to see a temp. change when fully heating. Consider that beds at higher
// temp. need longer to rise and cold beds need some time to get the temp. to
// the sensor. Time is in milliseconds! Set 0 to disable
...
/*
If the firmware is in laser mode, it can control a laser output to cut or
engrave materials. Please use this feature only if you know about safety and
required protection. Lasers are dangerous and can hurt or make you blind!!!

The default laser driver only supports laser on and off. Here you control the
intensity with your feedrate. For exchangeable diode lasers this is normally
enough. If you need more control you can set the intensity in a range 0-255 with
a custom extension to the driver. See driver.h and comments on how to extend the
functions non invasive with our event system.

If you have a laser - powder system you will like your E override. If moves
contain a increasing extruder position it will laser that move. With this trick
you can use existing FDM slicers to laser the output. Laser width is extrusion
width.

Other tools may use M3 and M5 to enable/disable laser. Here G1/G2/G3 moves have
laser enabled and G0 moves have it disables.
...
// If your axes are only moving in one direction, make sure the endstops are
// connected properly. If your axes move in one direction ONLY when the endstops
// are triggered, set ENDSTOPS_INVERTING to true here
...
/* If you want to keep z motor running on stepper timeout, remove comments
  below. This may be useful if your z bed moves when motors are disabled. Will
  still turn z off when heaters get also disabled.
*/
...
// If you do z min homing, you might want to rise extruder a bit after homing so
// it does not heat touching your bed.
...
/*  =========== Parameter essential for delta calibration ===================
            ...
            C, Y-Axis
            |                        |___| CARRIAGE_HORIZONTAL_OFFSET (recommend
   set it to 0) |                        |
   \------------------------------------------
            |_________ X-axis        |    \ | / \                       |     \
   DELTA_DIAGONAL_ROD (length)    Each move this Rod Height /   \ \ is
   calculated /     \                             \    Carriage is at printer
   center!   | A      B \_____/--------------------------------
                                              |--|
   END_EFFECTOR_HORIZONTAL_OFFSET (recommend set it to 0)
                                         |----| ROD_RADIUS (Horizontal rod pivot
   to pivot measure)
                                     |-----------| PRINTER_RADIUS (recommend set
   it to ROD_RADIUS)
...
// hitting the floor set this to zero. Otherwise, measure how high the carriage
// is from horizontal rod Also, movement speeds are 10x to 20x cartesian speeds
// at tower bottom. You may need to leave a few mm for safety. Hitting floor at
// high speed can damage your printer (motors, drives, etc) THIS MAY NEED
// UPDATING IF THE HOT END HEIGHT CHANGES!
...
/** \brief Printer radius in mm,
  ...
  Alternately set this to the pivot to pivot horizontal rod distance, when head
  is at (0,0)
*/
...
/** When true the delta will home to z max when reset/powered over cord. That
way you start with well defined coordinates. If you don't do it, make sure to
home first before your first move.
*/
#define DELTA_HOME_ON_POWER 0

/** To allow software correction of misaligned endstops, you can set the
correction in steps here. If you have EEPROM enabled you can also change the
values online and autoleveling will store the results here. */
...
/** \brief Number of delta moves in each line. Moves that exceed this figure
will be split into multiple lines. Increasing this figure can use a lot of
memory since 7 bytes * size of line buffer * MAX_SELTA_SEGMENTS_PER_LINE will be
allocated for the delta buffer. PrintLine PrintLine::lines[PRINTLINE_CACHE_SIZE
(default 16?)]; Printline is about 200 bytes + 7 * DELTASEGMENTS_PER_PRINTLINE
 ...
 * right measurement. In this case set HOME_ORDER_ZXYTZ and also define
...
printer, the bed will be pushed down by it's
...
/** If your driver needs some additional delay between setting direction and
 ...
 direction and signal, but some drivers might be even slower or you are using a
 fast Arduino board with slow driver. Normally 0 works. If you get skewed print,
 you might try 1 microsecond here.
 */
#define DIRECTION_DELAY 0
/** The firmware can only handle 16000Hz interrupt frequency cleanly. If you
need higher speeds a faster solution is needed, and this is to double/quadruple
the steps in one interrupt call. This is like reducing your 1/16th microstepping
to 1/8 or 1/4. It is much cheaper then 1 or 3 additional stepper interrupts with
all it's overhead. As a result you can go as high as 40000Hz.
*/
#define STEP_DOUBLER_FREQUENCY 12000
/** If you need frequencies off more then 30000 you definitely need to enable
this. If you have only 1/8 stepping enabling this may cause to stall your moves
when 20000Hz is reached.
*/
#define ALLOW_QUADSTEPPING 1
/** If you reach STEP_DOUBLER_FREQUENCY the firmware will do 2 or 4 steps with
nearly no delay. That can be too fast for some printers causing an early stall.
...
/** \brief X, Y, Z max acceleration in mm/s^2 for printing moves or retracts.
 Make sure your printer can go that high! Overridden if EEPROM activated.
*/
#define MAX_ACCELERATION_UNITS_PER_SQ_SECOND_X 1000
#define MAX_ACCELERATION_UNITS_PER_SQ_SECOND_Y 1000
#define MAX_ACCELERATION_UNITS_PER_SQ_SECOND_Z 100

/** \brief X, Y, Z max acceleration in mm/s^2 for travel moves.  Overridden if
 * EEPROM activated.*/
...
segments. Its unit is mm/s. If the printer is standing still, the start speed is
jerk/2. At the join of two segments, the speed difference is limited to the jerk
value.
...
v_diff > jerk => vx_1 = vy_2 = jerk/v_diff*vx_1 = 40/70.71*50 = 28.3 mm/s at the
join
...
increase this. Especially on many very short moves the cache may go empty. The
minimum value is 5.
*/
...
If the cache contains less then MOVE_CACHE_LOW segments, the time per segment is
limited to LOW_TICKS_PER_MOVE clock cycles. If a move would be shorter, the
feedrate will be reduced. This should prevent buffer underflows. Set this to 0
if you don't care about empty buffers during print.
*/
#define MOVE_CACHE_LOW 10
/** \brief Cycles per move, if move cache is low.

This value must be high enough, that the buffer has time to fill up. The problem
only occurs at the beginning of a print or if you are printing many very short
segments at high speed. Higher delays here allow higher values in
...
This is a safety value. If your extruder temperature is below this temperature,
no extruder steps are executed. This is to prevent your extruder to move unless
the filament is at least molten. After having some complains that the extruder
does not work, I leave it 0 as default.
*/
...
Without a correct adjusted advance algorithm, you get blobs at points, where
acceleration changes. The effect increases with speed and acceleration
difference. Using the advance method decreases this effect. For more
informations, read the wiki.
*/
#define USE_ADVANCE 1

/** \brief enables quadratic component.

Set 1 to allow, 0 disallow a quadratic advance dependency. Linear is the
dominant value, so no real need to activate the quadratic term. Only adds lots
of computations and storage usage. */
...
- 250000 : Fastest with error rate of 0% with 16 or 32 MHz - update
wiring_serial.c in your board files. See boards/readme.txt
- 115200 : Fast, but may produce communication errors on quite regular basis,
Error rate -3,5%
- 76800 : Best setting for Arduino with 16 MHz, Error rate 0,2% page 198 AVR1284
Manual. Result: Faster communication then 115200
- 57600 : Should produce nearly no errors, on my gen 6 it's faster than 115200
because there are no errors slowing down the connection
- 38600
...
/**
If you use an ATX power supply you need the power pin to work non inverting. For
some special boards you might need to make it inverting.
*/
...
/** What shall the printer do, when it receives an M112 emergency stop signal?
 ...
 1 = restart by resetting the AVR controller. The USB connection will not reset
 if managed by a different chip!
*/
...
software to send the next command. Not receiving it will cause your printer to
stop. Sending this string every second, if our queue is empty should prevent
this. Comment it, if you don't want this feature. */
#define WAITING_IDENTIFIER "wait"

/** \brief Sets time for echo debug

You can set M111 1 which enables ECHO of commands sent. This define specifies
the position, when it will be executed. In the original FiveD software, echo is
done after receiving the command. With checksum you know, how it looks from the
sending string. With this define uncommented, you will see the last command
executed. To be more specific: It is written after execution. This helps
tracking errors, because there may be 8 or more commands in the queue and it is
elsewise difficult to know, what your reprap is currently doing.
*/
...
epr_init() it will detect a mismatch of the first byte and copy default values
into EEPROM. If the first byte matches, the stored values are used to overwrite
the settings.

IMPORTANT: With mode <>0 some changes in Configuration.h are not set any more,
as they are taken from the EEPROM.
*/
#define EEPROM_MODE 2

/**************** duplicate motor driver ***************

If you have unused extruder steppers free, you could use it to drive the second
or third z motor instead of driving them with a single stepper. The same works
for the other axis if needed.
*/
...
- 0 is legacy mode. This is the way DUAL_X has always been working in Repetier.
With this mode you need to configure both extruders XOffset relative to X0 on
the bed. When toolchange command is issued, the current tool will be parked and
the new one will be unparked and positioned at the last X coordinate before the
switch, unless LAZY_DUAL_X_AXIS is set to 1, which will make the new extruder
stay parked until the next X,Y or Z move.

- 1 is the new mode. In this mode you need to adjust X_MIN_POS, X_MAX_LENGTH and
right extruder Xoffset to fit your particular setup. Usually, X_MIN_POS will be
a negative number which will be exactly the distance from left extruder home
position to bed X0. X_MAX_LENGTH will have to be the distance between left
carriage home position and just before hitting the right carriage in park
position, while XOffset of the right carriage will have to be the distance (in
steps) from right home position to bed X0. XOffset of left carriage must be 0.
  In this mode, when tool switch command is issued, the current extruder will be
parked and the new one will be unparked and positioned at the last X coordinate,
unless that coordinate is outside its reach (that would be the opposite carriage
parking spot). M606 command will be available to move the current carriage to a
position relative to its home position.
*/
...
sure all following settings are correct. You need this library to compile:
...
// and xy direction #define SENSORLESS_HOMING
...
If you need to control servos, enable this feature. You can control up to 4
servos. Control the servos with M340 P<servoId> S<pulseInUS> servoID = 0..3
Servos are controlled by a pulse width normally between 500 and 2500 with 1500ms
in center position. 0 turns servo off.

WARNING: Servos can draw a considerable amount of current. Make sure your system
can handle this or you may risk your hardware!
*/
...
This defines the full power duration before returning to set value. Time is in
milliseconds */
#define FAN_KICKSTART_TIME 200
/** Defines the max. fan speed for M106 controlled fans. Normally 255 to use
 full range, but for 12V fans on 24V this might help preventing a defect. For
 all other fans there is a explicit maximum PWM value you can set, so this is
 not used for other fans! */
...
/* How is z min measured
 ...
 For mode 1 the current coating thickness is added to measured z probe
 distances. That way the real bed is always the reference height. For inductive
 sensors or z min endstops the coating has no effect on the result, so you
 should use mode 0.
*/
...
quality we get comes from the selection of the right points and method.
...
/* Autoleveling allows it to z-probe 3 points to compute the inclination and
   compensates the error for the print. This feature requires a working z-probe
   and you should have z-endstop at the top not at the bottom. The same 3 points
   are used for the G29 command.
*/
...
can use this to compensate the effect of printing squares like parallelograms.
Set the parameter to then tangents of the deviation from 90° when you print a
square object. E.g. if you angle is 91° enter tan(1) = 0.017. If error doubles
you have the wrong sign. Always hard to say since the other angle is 89° in this
case!
*/
...
/** The GCODEs in this line get executed, when you stop a SD print before it was
ended. Separate commands by \n */
...
/* Speed when no cooling is required. Normally 0 but if you need slightly
cooling it can be set here */
#define BOARD_FAN_MIN_SPEED 0
/* You can have one additional fan controlled by a temperature. You can set
   set at which temperature it should turn on and at which it should reach max.
   speed.
*/
...
/** Adds support for ESP8266 Duet web interface, PanelDue and probably some
 * other things. This essentially adds command M36/M408 and extends M20. Since
 * it requires some memory do not enable it unless you have such a display!
 *  */
#define FEATURE_JSON 0

/** For displays and keys there are too many permutations to handle them all in
once. For the most common available combinations you can set the controller type
here, so you don't need to configure uicong.h at all. Controller settings > 1
disable usage of uiconfig.h

0 or NO_CONTROLLER = no display
1 or UICONFIG_CONTROLLER = Manual definition of display and keys parameter in
uiconfig.h

The following settings override uiconfig.h!
2 or CONTROLLER_SMARTRAMPS = Smartcontroller from reprapdiscount on a RAMPS or
RUMBA board 3 or CONTROLLER_ADAFRUIT = Adafruit RGB controller 4 or
CONTROLLER_FOLTYN = Foltyn 3DMaster with display attached 5 or CONTROLLER_VIKI =
ViKi LCD - Check pin configuration in ui.h for feature controller 5!!! sd card
disabled by default! 6 or CONTROLLER_MEGATRONIC = ReprapWorld Keypad / LCD,
predefined pins for Megatronics v2.0 and RAMPS 1.4. Please check if you have
used the defined pin layout in ui.h. 7 or CONTROLLER_RADDS = RADDS Extension
Port 8 or CONTROLLER_PIBOT20X4 = PiBot Display/Controller extension with 20x4
character display 9 or CONTROLLER_PIBOT16X2 = PiBot Display/Controller extension
with 16x2 character display 10 or CONTROLLER_GADGETS3D_SHIELD = Gadgets3D shield
on RAMPS 1.4, see
http://reprap.org/wiki/RAMPS_1.3/1.4_GADGETS3D_Shield_with_Panel 11 or
CONTROLLER_REPRAPDISCOUNT_GLCD = RepRapDiscount Full Graphic Smart Controller 12
or CONTROLLER_FELIX = FELIXPrinters Controller 13 or CONTROLLER_RAMBO = SeeMeCNC
Display on Rambo (ORION) 14 or CONTROLLER_OPENHARDWARE_LCD2004 =
OpenHardware.co.za LCD2004 V2014 15 or CONTROLLER_SANGUINOLOLU_PANELOLU2 =
Sanguinololu + Panelolu2 16 or CONTROLLER_GAMEDUINO2 (in development) 17 or
CONTROLLER_MIREGLI 17 18 or CONTROLLER_GATE_3NOVATICA Gate Controller from
3Novatica 21 or CONTROLLER_VIKI2 Panucatt VIKI2 graphic lcd 24 or
CONTROLLER_ZONESTAR = Zonestar P802M with LCD 20x4 and 5 ADC button keypad 25 or
CONTROLLER_ORCABOTXXLPRO2 26 or CONTROLLER_AZSMZ_12864 405 or
CONTROLLER_FELIX_DUE Felix LCD für due based board 27 or
...
/** How many ms should a single page be shown, until it is switched to the next
 * one.*/
#define UI_PAGES_DURATION 4000

/** Delay of start screen in milliseconds */
#define UI_START_SCREEN_DELAY 1000
/** Uncomment if you don't want automatic page switching. You can still switch
the info pages with next/previous button/click-encoder */
#define UI_DISABLE_AUTO_PAGESWITCH 1

/** Time to return to info menu if x milliseconds no key was pressed. Set to 0
 * to disable it. */
...
Unfortunately, the encoder have a different count of phase changes between
clicks. Select an encoder speed from 0 = fastest to 2 = slowest that results in
one menu move per click.
*/
...
/* There are 2 ways to change positions. You can move by increments of 1/0.1 mm
resulting in more menu entries and requiring many turns on your encode. The
alternative is to enable speed dependent positioning. It will change the move
distance depending on the speed you turn the encoder. That way you can move very
fast and very slow in the same setting.

*/
#define UI_SPEEDDEPENDENT_POSITIONING 1

/** If set to 1 faster turning the wheel makes larger jumps. Helps for faster
 * navigation. */
...
/*
If you have leveling with bed coating or fixed z min you can use this menu to
adjust 0 height with a simple bed coating menu which adds coating thickness.
*/
...
/*
#define USER_KEY1_PIN     UI_DISPLAY_D5_PIN      // D5 to display (not used for
graphics controller), change to other pin if you use character LCD ! #define
USER_KEY1_ACTION  UI_ACTION_FAN_SUSPEND #define USER_KEY2_PIN UI_DISPLAY_D6_PIN
// D6 to display (not used for graphics controller)... #define USER_KEY2_ACTION
UI_ACTION_SD_PRI_PAU_CONT #define USER_KEY3_PIN     UI_DISPLAY_D7_PIN      // D7
to display (not used for graphics controller)... #define USER_KEY3_ACTION
UI_ACTION_LIGHTS_ONOFF #define USER_KEY4_PIN     -1 #define USER_KEY4_ACTION
UI_ACTION_DUMMY
*/
...
// #define MOTOR_DRIVER_x StepperDriver<int stepPin, int dirPin, int
// enablePin,bool invertDir, bool invertEnable>(float stepsPerMM,float speed)
...
/*
  ...
  Read Events.h for more informations. To activate, uncomment the following
  define.
*/
