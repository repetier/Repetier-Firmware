...
/* 
    Work in progress sample configuration for the SKR E3 Mini v2.0.
...
// SKR E3 Mini - Assume we're using a BLTouch by default. 
...
// SKR E3 Mini - Stock creality 128x64 display uses an active buzzer, which is terrible.
// Using high-freq HW-PWM we can still run it like a passive buzzer anyway.
// (BEEPER_SOURCE_IO doesn't work well here since it's SW PWM. Artifacts etc.)
...
IO_INPUT_INVERTED_PULLUP(ControllerReset, UI_RESET_PIN)
#else
...
// SKR E3 Mini - Both extruder and bed heaters are on the same HW timer (TIM8),
...
// Fan outputs
// SKR E3 Mini V2 - Just use SW PWM for both fans @ 78Hz as well, since TIM8 and 
// TIM3 are in use. Could use TIM3 instead if you don't have a HW PWM beeper defined (TIM3)
// but both fans will share the same frequency.
IO_OUTPUT(IOFan1, FAN_PIN)
IO_PWM_SOFTWARE(Fan1PWMRaw, IOFan1, 1)
IO_PWM_KICKSTART(Fan1PWM, Fan1PWMRaw, 2, 50)
 
IO_OUTPUT(IOFan2, FAN2_PIN)
IO_PWM_SOFTWARE(Fan2PWMRaw, IOFan2, 1)
IO_PWM_KICKSTART(Fan2PWM, Fan2PWMRaw, 2, 50)
...
STEPPER_TMC2209_HW_UART(XMotor, IOX1Step, IOX1Dir, IOX1Enable, Serial3, 0.1f, 16, 550, 1, 0, 0, 0, 12, endstopNone, endstopNone)
STEPPER_TMC2209_HW_UART(YMotor, IOY1Step, IOY1Dir, IOY1Enable, Serial3, 0.1f, 16, 550, 1, 0, 0, 0, 12, endstopNone, endstopNone)
STEPPER_TMC2209_HW_UART(ZMotor, IOZ1Step, IOZ1Dir, IOZ1Enable, Serial3, 0.1f, 16, 550, 1, 0, 0, 0, 12, endstopNone, endstopNone)
STEPPER_TMC2209_HW_UART(E1MotorBase, IOE1Step, IOE1Dir, IOE1Enable, Serial3, 0.1f, 16, 650, 1, 0, 0, 0, 12, endstopNone, endstopNone)
