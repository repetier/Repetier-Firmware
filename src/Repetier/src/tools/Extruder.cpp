...
void Extruder::selectExtruderById(uint8_t extruderId) {
    ...
    current->extrudePosition = Motion1::currentPosition[E_AXIS];

#if RAISE_Z_ON_TOOLCHANGE > 0 && !LAZY_DUAL_X_AXIS
    if (executeSelect && Printer::isZHomed())
        PrintLine::moveRelativeDistanceInSteps(0, 0, static_cast<int32_t>(RAISE_Z_ON_TOOLCHANGE * Printer::axisStepsPerMM[Z_AXIS]), 0, Motion1::homingFeedrate[Z_AXIS], true, false);
#endif
    ...
    if (Motion1::isAxisHomed(X_AXIS) && executeSelect
#if LAZY_DUAL_X_AXIS
        && !Printer::sledParked
#endif
    ...
    if (Printer::isHomedAll() && next->zOffset < current->zOffset) { // prevent extruder from hitting bed - move bed down a bit
        Printer::offsetZ = -next->zOffset * Motion1::resolution[Z_AXIS];
        Printer::setNoDestinationCheck(true);
#if LAZY_DUAL_X_AXIS && DUAL_X_AXIS
        Printer::moveToReal((Printer::xMinSteps + current->xOffset) * Printer::invAxisStepsPerMM[X_AXIS], IGNORE_COORDINATE, IGNORE_COORDINATE, IGNORE_COORDINATE, Motion1::homingFeedrate[Z_AXIS]);
        Printer::sledParked = true;
#else
        Printer::moveToReal(IGNORE_COORDINATE, IGNORE_COORDINATE, IGNORE_COORDINATE, IGNORE_COORDINATE, Motion1::homingFeedrate[Z_AXIS]);
#endif
    ...
    Motion1::resolution[E_AXIS] = Extruder::current->stepsPerMM;
    // Printer::invAxisStepsPerMM[E_AXIS] = 1.0f / Printer::axisStepsPerMM[E_AXIS];
#endif
    Motion1::maxFeedrate[E_AXIS] = Extruder::current->maxFeedrate;
    //   max_start_speed_units_per_second[E_AXIS] = Extruder::current->maxStartFeedrate;
    Motion1::maxAcceleration[E_AXIS] = next->maxAcceleration;
#if USE_ADVANCE
...
#endif
    float fmax = ((float)HAL::maxExtruderTimerFrequency() / ((float)Printer::maxExtruderSpeed * Motion1::resolution[E_AXIS])); // Limit feedrate to interrupt speed
    if (fmax < Motion1::maxFeedrate[E_AXIS])
        Motion1::maxFeedrate[E_AXIS] = fmax;
#endif // USE_ADVANCE
    ...
    if (executeSelect) {
        if (Printer::isHomedAll()) { // prevent extruder from hitting bed - move bed down a bit
            ...
            Printer::moveToReal(IGNORE_COORDINATE, IGNORE_COORDINATE, cz, IGNORE_COORDINATE, Motion1::homingFeedrate[Z_AXIS]);
            Printer::sledParked = true;
            Commands::waitUntilEndOfAllMoves();
        }
...
#else // DUAL_X_AXIS
    Printer::offsetX = -next->xOffset / Motion1::resolution[X_AXIS];
#endif
    Printer::offsetY = -next->yOffset / Motion1::resolution[Y_AXIS];
    Printer::offsetZ = -next->zOffset / Motion1::resolution[Z_AXIS];
    ...
    if (wait && temperatureInCelsius > MAX_ROOM_TEMPERATURE
        ...
        do {
            ...
            if (actExtruder == Extruder::current && actExtruder->waitRetractUnits > 0 && !retracted && dirRising && actExtruder->tempControl.currentTemperatureC > actExtruder->waitRetractTemperature) {
                Motion1::setTmpPositionXYZE(0, 0, 0, -actExtruder->waitRetractUnits);
                Motion1::moveRelativeByOfficial(Motion1::tmpPosition, actExtruder->maxFeedrate * 0.25);
                retracted = 1;
            }
#endif
            if (maxWaitUntil == 0) {
...
#endif
        Motion1::setTmpPositionXYZE(0, 0, 0, -distance);
        Motion1::moveRelativeByOfficial(Motion1::tmpPosition, RMath::max(speed, 1.f));
#if MIXING_EXTRUDER
        if (!extraLength)
...
#ifdef EARLY_ZLIFT
            if (zLiftF > 0) {
                Motion1::setTmpPositionXYZ(0, 0, zLiftF);
                Motion1::moveRelativeByOfficial(Motion1::tmpPosition, Motion1::maxFeedrate[Z_AXIS]);
                Motion1::g92Offsets[Z_AXIS] -= zLiftF;
            }
#endif
...
#endif
      ,
      { 0, EXT0_TEMPSENSOR_TYPE, &TempExt1 /*EXT0_SENSOR_INDEX*/, EXT0_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT0_PID_INTEGRAL_DRIVE_MAX, EXT0_PID_INTEGRAL_DRIVE_MIN, EXT0_PID_PGAIN_OR_DEAD_TIME, EXT0_PID_I, EXT0_PID_D, EXT0_PID_MAX, 0, 0, 0, 0, 0, EXT0_DECOUPLE_TEST_PERIOD, 0, EXT0_PREHEAT_TEMP },
      ext0_select_cmd,
      ext0_deselect_cmd,
...
#endif
      ,
      { 1, EXT1_TEMPSENSOR_TYPE, &TempExt2 /*EXT1_SENSOR_INDEX*/, EXT1_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT1_PID_INTEGRAL_DRIVE_MAX, EXT1_PID_INTEGRAL_DRIVE_MIN, EXT1_PID_PGAIN_OR_DEAD_TIME, EXT1_PID_I, EXT1_PID_D, EXT1_PID_MAX, 0, 0, 0, 0, 0, EXT1_DECOUPLE_TEST_PERIOD, 0, EXT1_PREHEAT_TEMP },
      ext1_select_cmd,
      ext1_deselect_cmd,
