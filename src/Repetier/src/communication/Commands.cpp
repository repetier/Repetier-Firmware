...
void Commands::processMCode(GCode* com) {
    ...
    case 140: // M140 set bed temp
        ...
        if (Printer::debugDryrun())
            break;
        if (com->hasS())
            Extruder::setHeatedBedTemperature(com->S + (com->hasO() ? com->O : 0), com->hasF() && com->F > 0);
        else if (com->hasH())
            Extruder::setHeatedBedTemperature(heatedBedController.preheatTemperature + (com->hasO() ? com->O : 0), com->hasF() && com->F > 0);
#endif
        break;
...
#if NUM_EXTRUDER > 0
    {
        ...
        if (com->hasS())
            Extruder::setTemperatureForExtruder(com->S + (com->hasO() ? com->O : 0), actExtruder->id, com->hasF() && com->F > 0, true);
        else if (com->hasH())
            Extruder::setTemperatureForExtruder(actExtruder->tempControl.preheatTemperature + (com->hasO() ? com->O : 0), actExtruder->id, com->hasF() && com->F > 0, true);
    }
#endif
        previousMillisCmd = HAL::timeInMilliseconds();
        break;
