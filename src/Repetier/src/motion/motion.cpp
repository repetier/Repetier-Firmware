...
#endif
#else
        if(cur->isXMove()) XMotor.enable();
        if(cur->isYMove()) YMotor.enable();
        if(cur->isZMove()) ZMotor.enable();
#endif
...
#if !(GANTRY)
        XMotor.dir(cur->isXPositiveMove());
        YMotor.dir(cur->isYPositiveMove());
        ZMotor.dir(cur->isZPositiveMove());
...
#if FEATURE_Z_PROBE
        ZProbe->update();
        if(Printer::isZProbingActive() && cur->isZNegativeMove() && ZProbe->triggered()) {
            cur->setZMoveFinished();
            Printer::stepsRemainingAtZHit = cur->stepsRemaining;
        }
#endif
