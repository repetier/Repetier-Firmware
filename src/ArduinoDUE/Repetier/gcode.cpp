...
uint8_t GCode::commandsReceivingWritePosition =
    0;                       ///< Writing position in gcode_transbuffer.
uint8_t GCode::sendAsBinary; ///< Flags the command as binary input.
uint8_t GCode::commentDetected =
    false; ///< Flags true if we are reading the comment part of a command.
uint8_t
    GCode::binaryCommandSize; ///< Expected size of the incoming binary command.
bool GCode::waitUntilAllCommandsAreParsed =
    false; ///< Don't read until all commands are parsed. Needed if gcode_buffer
           ///< is misused as storage for strings.
uint32_t GCode::actLineNumber; ///< Line number of current command.
volatile uint8_t GCode::bufferLength =
...
int8_t GCode::waitingForResend =
    -1; ///< Waiting for line to be resend. -1 = no wait.
uint32_t GCode::lastLineNumber = 0; ///< Last line number received.
uint8_t GCode::wasLastCommandReceivedAsBinary =
    0; ///< Was the last successful command in binary mode?
millis_t GCode::timeOfLastDataPacket =
    0; ///< Time, when we got the last data packet. Used to detect missing
       ///< uint8_ts.
#endif
...
void GCode::checkAndPushCommand() {
...
#if NEW_COMMUNICATION
      GCodeSource::activeSource->lastLineNumber =
          0; // simulate a reset so lines are out of resend buffer
#else
  ...
  if (hasN()) {
#if NEW_COMMUNICATION
    if ((((GCodeSource::activeSource->lastLineNumber + 1) & 0xffff) !=
         (actLineNumber & 0xffff)))
#else
    if ((((lastLineNumber + 1) & 0xffff) != (actLineNumber & 0xffff)))
#endif
    {
#if NEW_COMMUNICATION
      if (static_cast<uint16_t>(GCodeSource::activeSource->lastLineNumber -
                                actLineNumber) < 40)
#else
...
#if NEW_COMMUNICATION
      else if (GCodeSource::activeSource->waitingForResend <
               0) // after a resend, we have to skip the garbage in buffers, no
                  // message for this
#else
...
void GCode::readFromSerial() {
  ...
  if (bufferLength >= GCODE_BUFFER_SIZE ||
      (waitUntilAllCommandsAreParsed && bufferLength)) {
  ...
  if (!GCodeSource::activeSource->dataAvailable()) {
    if (GCodeSource::activeSource
      ...
      if (commandsReceivingWritePosition >
          0) { // it's only an error if we have started reading a command
      ...
      if ((GCodeSource::activeSource->waitingForResend >= 0 ||
           commandsReceivingWritePosition > 0) &&
          time - GCodeSource::activeSource->timeOfLastDataPacket >
              200) // only if we get no further data after 200ms it is a problem
      {
        ...
        GCodeSource::activeSource->timeOfLastDataPacket = time;
               ...
               time - GCodeSource::activeSource->timeOfLastDataPacket >
                   1000) // Don't do it if buffer is not empty. It may be a slow
                         // executing command.
      {
    ...
    if (commandsReceivingWritePosition ==
        0) // nothing read, we can rotate to next input source
      GCodeSource::rotateSource();
  }
  while (GCodeSource::activeSource->dataAvailable() &&
         commandsReceivingWritePosition <
             MAX_CMD_SIZE) // consume data until no data or buffer full
  {
    GCodeSource::activeSource->timeOfLastDataPacket =
        time; // HAL::timeInMilliseconds();
    commandReceiving[commandsReceivingWritePosition++] =
        GCodeSource::activeSource->readByte();
    // first lets detect, if we got an old type ascii command
    if (commandsReceivingWritePosition == 1 && commentDetected == false) {
      if (GCodeSource::activeSource->waitingForResend >= 0 &&
          GCodeSource::activeSource->wasLastCommandReceivedAsBinary) {
    ...
    if (sendAsBinary) {
      if (commandsReceivingWritePosition < 2)
        continue;
      if (commandsReceivingWritePosition == 5 ||
          commandsReceivingWritePosition == 4)
    ...
    } else // ASCII command
    {
      ...
      if (ch == 0 || ch == '\n' || ch == '\r' ||
          !GCodeSource::activeSource
               ->isOpen() /*|| (!commentDetected && ch == ':')*/) // complete
                                                                  // line read
      {
  ...
  if (!HAL::serialByteAvailable()) {
    if ((waitingForResend >= 0 || commandsReceivingWritePosition > 0) &&
        time - timeOfLastDataPacket > 200) {
      // Com::printF(PSTR("WFR:"),waitingForResend);Com::printF(PSTR("
      // CRWP:"),commandsReceivingWritePosition);commandReceiving[commandsReceivingWritePosition]
      // = 0;Com::printFLN(PSTR(" GOT:"),(char*)commandReceiving);
      requestResend(); // Something is wrong, a started line was not continued
             ...
             MAX_CMD_SIZE) // consume data until no data or buffer full
  {
    ...
    } else // ASCII command
    {
      ...
      if (ch == 0 || ch == '\n' ||
          ch ==
              '\r' /*|| (!commentDetected && ch == ':')*/) // complete line read
      {
        commandReceiving[commandsReceivingWritePosition - 1] = 0;
#ifdef DEBUG_ECHO_ASCII
        Com::printF(PSTR("Got:"));
...
#if SDSUPPORT
  if (sd.sdmode == 0 || sd.sdmode >= 100 ||
      commandsReceivingWritePosition !=
          0) // not reading or incoming serial command
    return;
  while (sd.filesize > sd.sdpos &&
         commandsReceivingWritePosition <
             MAX_CMD_SIZE) // consume data until no data or buffer full
  {
    ...
    if (sendAsBinary) {
      if (commandsReceivingWritePosition < 2)
        continue;
      if (commandsReceivingWritePosition == 4 ||
          commandsReceivingWritePosition == 5)
        binaryCommandSize = computeBinarySize((char *)commandReceiving);
      if (commandsReceivingWritePosition == binaryCommandSize) {
        GCode *act = &commandsBuffered[bufferWriteIndex];
        if (act->parseBinary(
      ...
      char ch = commandReceiving[commandsReceivingWritePosition - 1];
      bool returnChar = ch == '\n' || ch == '\r';
      if (returnChar || sd.filesize == sd.sdpos ||
          (!commentDetected && ch == ':') ||
          commandsReceivingWritePosition >=
              (MAX_CMD_SIZE - 1)) // complete line read
      {
        if (returnChar || ch == ':')
          commandReceiving[commandsReceivingWritePosition - 1] = 0;
        else
...
bool GCode::parseAscii(char *line, bool fromSerial) {
  ...
  while ((c = *(pos++))) {
    ...
    case 'm': {
      ...
      if (M == 20 || M == 23 || M == 28 || M == 29 || M == 30 || M == 32 ||
          M == 36 || M == 117 || M == 531) {
...
#if NEW_COMMUNICATION
  if (GCodeSource::activeSource->wasLastCommandReceivedAsBinary &&
      !hasChecksum && fromSerial && !waitUntilAllCommandsAreParsed) {
#else
...
void SerialGCodeSource::prefetchContent() {
  ...
  while (stream->available() && bufLength < SERIAL_IN_BUFFER) {
    commandReceiving[commandsReceivingWritePosition++] = buffer[bufWritePos] =
        stream->read();
    bufWritePos++;
    ...
    if (sendAsBinary) {
      if (commandsReceivingWritePosition >= 2) {
        ...
        if (commandsReceivingWritePosition == 5 ||
            commandsReceivingWritePosition == 4)
          binaryCommandSize =
              GCode::computeBinarySize((char *)commandReceiving);
        if (commandsReceivingWritePosition == binaryCommandSize) {
          GCode act;
          if (act.parseBinary(commandReceiving, binaryCommandSize,
                              false)) { // Success
...
void SerialGCodeSource::testEmergency(GCode &gcode) {
  if (gcode.hasM()) {
    if (gcode.M == 108) {
      ...
      if (gcode.hasZ()) {
        if (abs(gcode.Z) < (32700 - labs(Printer::zBabystepsMissing)) *
                               Printer::axisStepsPerMM[Z_AXIS])
          Printer::zBabystepsMissing +=
              gcode.Z * Printer::axisStepsPerMM[Z_AXIS];
      }
#endif
    }
  }
...
FlashGCodeSource::FlashGCodeSource() : GCodeSource() { finished = true; }
