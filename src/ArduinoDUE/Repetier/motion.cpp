...
#if MOVE_X_WHEN_HOMED == 1 || MOVE_Y_WHEN_HOMED == 1 || MOVE_Z_WHEN_HOMED == 1
    if (!Printer::isHoming() && !Printer::isNoDestinationCheck())
    { // prevent movements when not homed
#if MOVE_X_WHEN_HOMED
        if (!Printer::isXHomed())
            x = 0;
#endif
...
void PrintLine::queueCartesianSegmentTo(uint8_t check_endstops, uint8_t pathOptimize)
{
    ...
    if ((p->isXYZMove()) && ((p->dir & XYZ_DIRPOS) ^ (Printer::backlashDir & XYZ_DIRPOS)) & (Printer::backlashDir >> 3))
    { // We need to compensate backlash, add a move
...
{
    if (stepsRemaining == 0)
    { // need at least one step for bresenham
        return;
    }
#if NONLINEAR_SYSTEM
    ...
    if (linesCount < MOVE_CACHE_LOW && timeForMove < LOW_TICKS_PER_MOVE)
    { // Limit speed to keep cache full.
    ...
    if (isZMove())
    {                                                                                           // normally no move in z direction
    ...
    if (axisDistanceMM[VIRTUAL_AXIS] >= 0)
    { // only for deltas all speeds in all directions have same limit
...
#define minTime 45000L
    ...
    {                 // Nothing to plan, only new element present
...
#endif
#endif
    if (static_cast<int32_t>(accelSteps + decelSteps) > stepsRemaining)
    { // can't reach limit speed
...
inline void PrintLine::backwardPlanner(ufast8_t start, ufast8_t last)
{
    ...
    while (start != last)
    {
        ...
        if (lastJunctionSpeed >= previous->maxJunctionSpeed)
        { // Limit is reached
...
{
    if (linesCount == 0 && waitRelax == 0 && pathOptimize)
    {   // First line after some time - warm up needed
        //return 0;
#if NONLINEAR_SYSTEM
        uint8_t w = 3;
#else
...
{
    while (getLinesCount() + b > PRINTLINE_CACHE_SIZE)
...
uint8_t PrintLine::queueNonlinearMove(uint8_t check_endstops, uint8_t pathOptimize, uint8_t softEndstop)
{
    ...
    for (int lineNumber = 1; lineNumber <= numLines; lineNumber++)
    {
        ...
        { // Is delta move or E axis leading
...
int32_t PrintLine::bresenhamStep()
{ // Version for delta printer
#if CPU_ARCH == ARCH_ARM
    if (!PrintLine::nlFlag)
#else
    if (cur == NULL)
#endif
    {
        ...
        if (Printer::debugNoMoves())
        { // simulate a move, but do nothing in reality
    ...
    if (curd != NULL)
    {
        if (curd->checkEndstops(cur, (cur->isCheckEndstops())))
        { // should stop move
            cur->stepsRemaining = 0;
            curd = NULL;
            ...
            { // Last move disables laser for safety!
    ...
    if (cur->stepsRemaining <= 0 || cur->isNoMove())
    {   // line finished
