...
float maxadvspeed = 0;
#endif
uint8_t pwm_pos[NUM_PWM]; // 0-NUM_EXTRUDER = Heater 0-NUM_EXTRUDER of extruder,
                          // NUM_EXTRUDER = Heated bed, NUM_EXTRUDER+1 Board
                          // fan, NUM_EXTRUDER+2 = Fan
volatile int waitRelax =
    0; // Delay filament relax at the end of print, could be a simple timeout
...
#endif
ufast8_t PrintLine::linesWritePos =
    0; ///< Position where we write the next cached line move.
volatile ufast8_t PrintLine::linesCount =
    0;                            ///< Number of lines cached 0 = nothing to do.
ufast8_t PrintLine::linesPos = 0; ///< Position for executing line movement.

/**
Move printer the given number of steps. Puts the move into the queue. Used by
e.g. homing commands. Does not consider rotation but updates position correctly
considering rotation. This can be used to correct positions when changing tools.
...
*/
void PrintLine::moveRelativeDistanceInSteps(int32_t x, int32_t y, int32_t z,
                                            int32_t e, float feedrate,
                                            bool waitEnd, bool checkEndstop,
                                            bool pathOptimize) {
  Printer::unparkSafety();
#if NUM_EXTRUDER > 0
  if (Printer::debugDryrun() ||
      (MIN_EXTRUDER_TEMP > 30 &&
       Extruder::current->tempControl.currentTemperatureC < MIN_EXTRUDER_TEMP &&
       !Printer::isColdExtrusionAllowed() &&
       Extruder::current->tempControl.sensorType != 0))
...
#endif //  MOVE_X_WHEN_HOMED == 1 || MOVE_Y_WHEN_HOMED == 1 || MOVE_Z_WHEN_HOMED
       //  == 1
  float savedFeedrate = Printer::feedrate;
  Printer::destinationPositionTransformed[X_AXIS] =
      Printer::currentPositionTransformed[X_AXIS] +
      x * Printer::invAxisStepsPerMM[X_AXIS];
  Printer::destinationPositionTransformed[Y_AXIS] =
      Printer::currentPositionTransformed[Y_AXIS] +
      y * Printer::invAxisStepsPerMM[Y_AXIS];
  Printer::destinationPositionTransformed[Z_AXIS] =
      Printer::currentPositionTransformed[Z_AXIS] +
      z * Printer::invAxisStepsPerMM[Z_AXIS];
  Printer::destinationPositionTransformed[E_AXIS] =
      Printer::currentPositionTransformed[E_AXIS] +
      e * Printer::invAxisStepsPerMM[E_AXIS];
  ...
  if (!queueNonlinearMove(checkEndstop, pathOptimize, false)) {
    Com::printWarningFLN(
        PSTR("moveRelativeDistanceInSteps / queueDeltaMove returns error"));
  }
#else
#if DISTORTION_CORRECTION
  Printer::destinationSteps[Z_AXIS] -=
      Printer::zCorrectionStepsIncluded; // correct as it will be added later in
                                         // Cartesian move computation
#endif
...
/** Adds the steps converted to mm to the lastCmdPos position and moves to that
position using Printer::moveToReal. Will use Printer::isPositionAllowed to
...
\param pathOptimize If false start and end speeds get fixed to minimum values.
*/
void PrintLine::moveRelativeDistanceInStepsReal(int32_t x, int32_t y, int32_t z,
                                                int32_t e, float feedrate,
                                                bool waitEnd,
                                                bool pathOptimize) {
  Printer::unparkSafety();
#if MOVE_X_WHEN_HOMED == 1 || MOVE_Y_WHEN_HOMED == 1 || MOVE_Z_WHEN_HOMED == 1
  if (!Printer::isHoming() &&
      !Printer::isNoDestinationCheck()) { // prevent movements when not homed
#if MOVE_X_WHEN_HOMED
    if (!Printer::isXHomed())
      x = 0;
#endif
  ...
  if (!Printer::isPositionAllowed(Printer::lastCmdPos[X_AXIS],
                                  Printer::lastCmdPos[Y_AXIS],
                                  Printer::lastCmdPos[Z_AXIS])) {
    return; // ignore move
  }
#if NUM_EXTRUDER > 0
  if (Printer::debugDryrun() ||
    ...
    e = 0; // should not be allowed for current temperature
#endif
  Printer::moveToReal(Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS],
                      Printer::lastCmdPos[Z_AXIS],
                      (Printer::currentPositionSteps[E_AXIS] + e) *
                          Printer::invAxisStepsPerMM[E_AXIS],
                      feedrate, pathOptimize);
...
#if !NONLINEAR_SYSTEM
#if DISTORTION_CORRECTION
/* Special version which adds distortion correction to z. Gets called from
 * queueCartesianMove if needed. */
void PrintLine::queueCartesianSegmentTo(uint8_t check_endstops,
                                        uint8_t pathOptimize) {

  // Correct the bumps
  Printer::zCorrectionStepsIncluded = Printer::distortion.correct(
      Printer::destinationSteps[X_AXIS], Printer::destinationSteps[Y_AXIS],
      Printer::destinationSteps[Z_AXIS]);
  Printer::destinationSteps[Z_AXIS] += Printer::zCorrectionStepsIncluded;
#if DEBUG_DISTORTION
  Com::printF(PSTR("zCorr:"),
              Printer::zCorrectionStepsIncluded *
                  Printer::invAxisStepsPerMM[Z_AXIS],
              3);
  Com::printF(PSTR(" atX:"), Printer::destinationSteps[X_AXIS] *
                                 Printer::invAxisStepsPerMM[X_AXIS]);
  Com::printFLN(PSTR(" atY:"), Printer::destinationSteps[Y_AXIS] *
  ...
  for (uint8_t axis = 0; axis < E_AXIS_ARRAY; axis++) {
    p->delta[axis] =
        Printer::destinationSteps[axis] - Printer::currentPositionSteps[axis];
    p->secondSpeed = Printer::fanSpeed;
    // axisDistanceMM[axis] = p->delta[axis] * Printer::invAxisStepsPerMM[axis];
    if (axis == E_AXIS) {
      axisDistanceMM[axis] = (Printer::destinationPositionTransformed[axis] -
                              Printer::currentPositionTransformed[axis]) *
                             Printer::extrusionFactor;
      if (Printer::mode == PRINTER_MODE_FFF) {
        Printer::extrudeMultiplyError +=
            axisDistanceMM[axis] * Printer::axisStepsPerMM[E_AXIS];
        p->delta[E_AXIS] = lroundf(Printer::extrudeMultiplyError);
        Printer::extrudeMultiplyError -= p->delta[E_AXIS];
        Printer::filamentPrinted +=
            p->delta[E_AXIS] * Printer::invAxisStepsPerMM[axis];
      }
#if defined(SUPPORT_LASER) && SUPPORT_LASER
      else if (Printer::mode == PRINTER_MODE_LASER) {
        p->secondSpeed =
            ((p->delta[X_AXIS] != 0 || p->delta[Y_AXIS] != 0) &&
                     (LaserDriver::laserOn || p->delta[E_AXIS] != 0)
    ...
    Printer::currentPositionTransformed[axis] =
        Printer::destinationPositionTransformed[axis];
  }
  if (p->isNoMove()) {
    if (newPath) { // need to delete dummy elements, otherwise commands can get
                   // locked.
  ...
  if ((p->isXYZMove()) &&
      ((p->dir & XYZ_DIRPOS) ^ (Printer::backlashDir & XYZ_DIRPOS)) &
          (Printer::backlashDir >>
           3)) { // We need to compensate backlash, add a move
    ...
    back_diff[X_AXIS] =
        (changed & 1
             ? (p->isXPositiveMove() ? Printer::backlashX : -Printer::backlashX)
             : 0);
    back_diff[Y_AXIS] =
        (changed & 2
             ? (p->isYPositiveMove() ? Printer::backlashY : -Printer::backlashY)
             : 0);
    back_diff[Z_AXIS] =
        (changed & 4
             ? (p->isZPositiveMove() ? Printer::backlashZ : -Printer::backlashZ)
             : 0);
    ...
    // Define variables that are needed for the Bresenham algorithm. Please note
    // that  Z is not currently included in the Bresenham algorithm.
    if (p->delta[Y_AXIS] > p->delta[X_AXIS] &&
        p->delta[Y_AXIS] > p->delta[Z_AXIS])
    ...
    xydist2 = back_diff[X_AXIS] * back_diff[X_AXIS] +
              back_diff[Y_AXIS] * back_diff[Y_AXIS];
    if (p->isZMove())
  ...
  if (p->delta[Y_AXIS] > p->delta[X_AXIS] &&
      p->delta[Y_AXIS] > p->delta[Z_AXIS] &&
      p->delta[Y_AXIS] > p->delta[E_AXIS])
    p->primaryAxis = Y_AXIS;
  else if (p->delta[X_AXIS] > p->delta[Z_AXIS] &&
           p->delta[X_AXIS] > p->delta[E_AXIS])
    p->primaryAxis = X_AXIS;
  ...
  if (p->isXYZMove()) {
    xydist2 = axisDistanceMM[X_AXIS] * axisDistanceMM[X_AXIS] +
              axisDistanceMM[Y_AXIS] * axisDistanceMM[Y_AXIS];
    if (p->isZMove())
      p->distance =
          RMath::max((float)sqrt(xydist2 + axisDistanceMM[Z_AXIS] *
                                               axisDistanceMM[Z_AXIS]),
                     fabs(axisDistanceMM[E_AXIS]));
    else
      p->distance =
          RMath::max((float)sqrt(xydist2), fabs(axisDistanceMM[E_AXIS]));
  } else
...
/**
  ...
  destinationSteps must be excluding any z correction! We will add that if
  required here.

  @param check_endstops Read end stop during move.
*/
void PrintLine::queueCartesianMove(uint8_t check_endstops,
                                   uint8_t pathOptimize) {
  ENSURE_POWER
#if LAZY_DUAL_X_AXIS
  if (Printer::sledParked && (Printer::currentPositionSteps[X_AXIS] !=
                                  Printer::destinationSteps[X_AXIS] ||
                              Printer::currentPositionSteps[Y_AXIS] !=
                                  Printer::destinationSteps[Y_AXIS] ||
  ...
  if (Printer::distortion.isEnabled() &&
      Printer::destinationSteps[Z_AXIS] < Printer::distortion.zMaxSteps() &&
      Printer::isZProbingActive() == false && !Printer::isHoming()) {
    // we are inside correction height so we split all moves in lines of max. 10
    // mm and add them including a z correction
    ...
    for (fast8_t i = 0; i < E_AXIS_ARRAY; i++) {
      ...
      fdeltas[i] = Printer::destinationPositionTransformed[i] -
                   Printer::currentPositionTransformed[i];
    ...
    for (int i = 1; i <= segments; i++) {
      for (fast8_t j = 0; j < E_AXIS_ARRAY; j++) {
        ...
        Printer::destinationPositionTransformed[j] =
            fstart[j] + (i * fdeltas[j]) / segments;
      }
      queueCartesianSegmentTo(check_endstops, pathOptimize);
    }
    return;
...
void PrintLine::calculateMove(float axisDistanceMM[], uint8_t pathOptimize,
                              fast8_t drivingAxis) {
  if (stepsRemaining == 0) { // need at least one step for bresenham
    return;
  }
#if NONLINEAR_SYSTEM
  long axisInterval[VIRTUAL_AXIS_ARRAY]; // shortest interval possible for that
                                         // axis
#else
  long axisInterval[E_AXIS_ARRAY];
#endif
  // float timeForMove = (float)(F_CPU)*distance / (isXOrYMove() ?
  // RMath::max(Printer::minimumSpeed, Printer::feedrate) : Printer::feedrate);
  // // time is in ticks
  float timeForMove =
      (float)(F_CPU)*distance / Printer::feedrate; // time is in ticks
  // bool critical = Printer::isZProbingActive();
  if (linesCount < MOVE_CACHE_LOW &&
      timeForMove < LOW_TICKS_PER_MOVE) { // Limit speed to keep cache full.
    // Com::printF(PSTR("L:"),(int)linesCount);
    // Com::printF(PSTR(" Old "),timeForMove);
    timeForMove += (3 * (LOW_TICKS_PER_MOVE - timeForMove)) /
                   (linesCount + 1); // Increase time if queue gets empty. Add
                                     // more time if queue gets smaller.
    // Com::printFLN(PSTR("Slow "),timeForMove);
    // critical = true;
  }
  timeInTicks = timeForMove;
  UI_MEDIUM; // do check encoder
  // Compute the slowest allowed interval (ticks/step), so maximum feedrate is
  // not violated
  int32_t limitInterval0;
  int32_t limitInterval = limitInterval0 =
      timeForMove / stepsRemaining; // until not violated by other constraints
                                    // it is your target speed
  float toTicks = static_cast<float>(F_CPU) / stepsRemaining;
  if (isXMove()) {
    axisInterval[X_AXIS] =
        axisDistanceMM[X_AXIS] * toTicks /
        (Printer::maxFeedrate[X_AXIS]); // mm*ticks/s/(mm/s*steps) = ticks/step
  ...
  if (isYMove()) {
    axisInterval[Y_AXIS] =
        axisDistanceMM[Y_AXIS] * toTicks / Printer::maxFeedrate[Y_AXIS];
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
  ...
  if (isZMove()) { // normally no move in z direction
    axisInterval[Z_AXIS] =
        axisDistanceMM[Z_AXIS] * toTicks /
        Printer::maxFeedrate[Z_AXIS]; // must prevent overflow!
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
  ...
  if (isEMove()) {
    axisInterval[E_AXIS] =
        axisDistanceMM[E_AXIS] * toTicks / Printer::maxFeedrate[E_AXIS];
    limitInterval = RMath::max(axisInterval[E_AXIS], limitInterval);
  } else
    axisInterval[E_AXIS] = 0;
#if DRIVE_SYSTEM == DELTA
  if (axisDistanceMM[VIRTUAL_AXIS] >=
      0) { // only for deltas all speeds in all directions have same limit
    axisInterval[VIRTUAL_AXIS] =
        axisDistanceMM[VIRTUAL_AXIS] * toTicks / (Printer::maxFeedrate[Z_AXIS]);
  ...
  fullInterval = limitInterval =
      limitInterval > LIMIT_INTERVAL
          ? limitInterval
          : LIMIT_INTERVAL; // This is our target speed
  if (limitInterval != limitInterval0) {
    // new time at full speed = limitInterval*p->stepsRemaining [ticks]
    timeForMove = (float)limitInterval *
                  (float)stepsRemaining; // for large z-distance this overflows
                                         // with long computation
  }
  float inverseTimeS = (float)F_CPU / timeForMove;
  if (isXMove()) {
    axisInterval[X_AXIS] =
        static_cast<int32_t>(timeForMove / (axisDistanceMM[X_AXIS] *
                                            Printer::axisStepsPerMM[X_AXIS]));
  ...
  if (isYMove()) {
    axisInterval[Y_AXIS] =
        static_cast<int32_t>(timeForMove / (axisDistanceMM[Y_AXIS] *
                                            Printer::axisStepsPerMM[Y_AXIS]));
  ...
  if (isZMove()) {
    axisInterval[Z_AXIS] =
        static_cast<int32_t>(timeForMove / (axisDistanceMM[Z_AXIS] *
                                            Printer::axisStepsPerMM[Z_AXIS]));
  ...
  if (isEMove()) {
    axisInterval[E_AXIS] =
        static_cast<int32_t>(timeForMove / (axisDistanceMM[E_AXIS] *
                                            Printer::axisStepsPerMM[E_AXIS]));
  ...
  // long interval = axis_interval[primary_axis]; // time for every step in
  // ticks with full speed If acceleration is enabled, do some Bresenham
  // calculations depending on which axis will lead it.
#if RAMP_ACCELERATION
  // slowest time to accelerate from v0 to limitInterval determines used
  // acceleration t = (v_end-v_start)/a
  float slowestAxisPlateauTimeRepro =
      1e15; // 1/time to reduce division Unit: 1/s
  uint32_t *accel =
      (isEPositiveMove() ? Printer::maxPrintAccelerationStepsPerSquareSecond
                         : Printer::maxTravelAccelerationStepsPerSquareSecond);
#if defined(INTERPOLATE_ACCELERATION_WITH_Z) &&                                \
    INTERPOLATE_ACCELERATION_WITH_Z != 0
  uint32_t newAccel[4];
  float accelFac =
      (100.0 + (EEPROM::accelarationFactorTop() - 100.0) *
                   Printer::currentPosition[Z_AXIS] / Printer::zLength) *
      0.01;
  ...
  for (fast8_t i = 0; i < E_AXIS_ARRAY; i++) {
    if (isMoveOfAxis(i))
      ...
      slowestAxisPlateauTimeRepro =
          RMath::min(slowestAxisPlateauTimeRepro,
                     (float)axisInterval[i] *
                         (float)accel[i]); //  steps/s^2 * step/tick  Ticks/s^2
  }

  // Errors for delta move are initialized in timer (except extruder)
#if !NONLINEAR_SYSTEM
  error[X_AXIS] = error[Y_AXIS] = error[Z_AXIS] = error[E_AXIS] =
      delta[primaryAxis] >> 1;
#endif
#if NONLINEAR_SYSTEM
  ...
  accelerationPrim =
      slowestAxisPlateauTimeRepro /
      axisInterval[primaryAxis]; // a = v/t = F_CPU/(c*t): Steps/s^2
  // Now we can calculate the new primary axis acceleration, so that the slowest
  // axis max acceleration is not violated
  fAcceleration = 262144.0 * (float)accelerationPrim /
                  F_CPU; // will overflow without float!
  accelerationDistance2 = 2.0 * distance * slowestAxisPlateauTimeRepro *
                          fullSpeed / ((float)F_CPU); // mm^2/s^2
  ...
  vMax = F_CPU /
         fullInterval; // maximum steps per second, we can reach
  ...
  if (!isXYZMove() || !isEPositiveMove()) {
    ...
    float advlin = fabs(speedE) * Extruder::current->advanceL * 0.001 *
                   Printer::axisStepsPerMM[E_AXIS];
    advanceL = ((65536L * advlin) / vMax); // advanceLscaled =
                                           // (65536*vE*k2)/vMax
#if ENABLE_QUADRATIC_ADVANCE
    advanceFull = 65536 * Extruder::current->advanceK * speedE *
                  speedE; // Steps*65536 at full speed
    long steps = (HAL::U16SquaredToU32(vMax)) /
                 (accelerationPrim
                  << 1); // v^2/(2*a) = steps needed to accelerate from 0-vMax
...
It goes from the last entry and tries to increase the end speed of previous
moves in a fashion that the maximum jerk is never exceeded. If a segment with
reached maximum speed is met, the planner stops. Everything left from this is
already optimal from previous updates. The first 2 entries in the queue are not
checked. The first is the one that is already in print and the following will
...
#define minTime 45000L
  ...
  firstLine->block(); // don't let printer touch this or following segments
                      // during update
  noInts.unprotect();
  ufast8_t previousIndex = linesWritePos;
  previousPlannerIndex(previousIndex);
  PrintLine *previous =
      &lines[previousIndex]; // segment before the one we are inserting
#if DRIVE_SYSTEM != DELTA
  // filters z-move<->not z-move
  /*    if((previous->primaryAxis == Z_AXIS && act->primaryAxis != Z_AXIS) ||
     (previous->primaryAxis != Z_AXIS && act->primaryAxis == Z_AXIS))
      {
  ...
  if (previous->isEOnlyMove() != act->isEOnlyMove()) {
    previous->maxJunctionSpeed =
        previous->endSpeed; // act->startSpeed; // maybe remove this. Previous
                            // should be at minimum and systems have nothing in
                            // common
    previous->setEndSpeedFixed(true);
    act->setStartSpeedFixed(true);
    act->updateStepsParameter();
    ...
    computeMaxJunctionSpeed(
        previous,
        act); // Set maximum junction speed if we have a real move before
  }
...
#endif
    // noInts.protect();
    lines[first].unblock(); // start with first block to release next used
...
optimal conditions. There is no guarantee that the previous move will be able to
reach the speed at all, but if it could exceed it will never exceed this
theoretical limit.
...
*/
inline void PrintLine::computeMaxJunctionSpeed(PrintLine *previous,
                                               PrintLine *current) {
#if NONLINEAR_SYSTEM
  /*  if (previous->moveID == current->moveID)   // Avoid computing junction
    speed for split nonlinear lines
    {
  ...
  if (Printer::isAdvanceActivated()) {
    // if we start/stop extrusion we need to do so with lowest possible end
    // speed or advance would leave a drolling extruder and can not adjust fast
    // enough.
    if (previous->isEMove() != current->isEMove()) {
      ...
      previous->endSpeed = current->startSpeed = previous->maxJunctionSpeed =
          RMath::min(previous->endSpeed, current->startSpeed);
  ...
  if (previous->distance < MAX_JERK_DISTANCE)
    lengthFactor = static_cast<float>(MAX_JERK_DISTANCE * MAX_JERK_DISTANCE) /
                   (previous->distance * previous->distance);
#endif
  float maxJoinSpeed = RMath::min(current->fullSpeed, previous->fullSpeed);
#if (DRIVE_SYSTEM ==                                                           \
     DELTA) // No point computing Z Jerk separately for delta moves
#ifdef ALTERNATIVE_JERK
  float jerk = maxJoinSpeed * lengthFactor *
               (1.0 - (current->speedX * previous->speedX +
                       current->speedY * previous->speedY +
                       current->speedZ * previous->speedZ) /
                          (current->fullSpeed * previous->fullSpeed));
#else
...
#if CPU_ARCH == ARCH_AVR
    accelSteps =
        ((vmax2 - HAL::U16SquaredToU32(vStart)) / (accelerationPrim << 1)) +
        1; // Always add 1 for missing precision
#else
    accelSteps = ((vmax2 - static_cast<uint64_t>(vStart) *
...
#if CPU_ARCH == ARCH_AVR
    decelSteps =
        ((vmax2 - HAL::U16SquaredToU32(vEnd)) / (accelerationPrim << 1)) + 1;
#else
    decelSteps =
        ((vmax2 - static_cast<uint64_t>(vEnd) * static_cast<uint64_t>(vEnd)) /
         (accelerationPrim << 1)) +
        1;
#endif
  }
...
#endif
#endif
  if (static_cast<int32_t>(accelSteps + decelSteps) >
      stepsRemaining) { // can't reach limit speed
    uint32_t red = (accelSteps + decelSteps - stepsRemaining) >> 1;
    accelSteps = accelSteps - RMath::min(static_cast<int32_t>(accelSteps),
                                         static_cast<int32_t>(red));
    decelSteps = decelSteps - RMath::min(static_cast<int32_t>(decelSteps),
                                         static_cast<int32_t>(red));
  }
...
The backwards planner traverses the moves from last to first looking at
deceleration. The RHS of the accelerate/decelerate ramp.
...
inline void PrintLine::backwardPlanner(ufast8_t start, ufast8_t last) {
  ...
  // PREVIOUS_PLANNER_INDEX(last); // Last element is already fixed in start
  // speed
  while (start != last) {
...
#if NONLINEAR_SYSTEM
    /*if (previous->moveID == act->moveID && lastJunctionSpeed ==
    previous->maxJunctionSpeed)
    {
        act->startSpeed = RMath::max(act->minSpeed, previous->endSpeed =
    lastJunctionSpeed); previous->invalidateParameter();
        act->invalidateParameter();
    }*/
#endif

    /* if(prev->isEndSpeedFixed())   // Nothing to update from here on, happens
     when path optimize disabled
     {
         act->setStartSpeedFixed(true);
         return;
     }*/
    ...
    lastJunctionSpeed =
        (act->isNominalMove()
             ? act->fullSpeed
             : sqrt(lastJunctionSpeed * lastJunctionSpeed +
                    act->accelerationDistance2)); // acceleration is
                                                  // acceleration*distance*2!
                                                  // What can be reached if we
                                                  // try?
    ...
    if (lastJunctionSpeed >= previous->maxJunctionSpeed) { // Limit is reached
      // If the previous line's end speed has not been updated to maximum speed
      // then do it now
      if (previous->endSpeed != previous->maxJunctionSpeed) {
        previous->invalidateParameter(); // Needs recomputation
        previous->endSpeed =
            RMath::max(previous->minSpeed,
                       previous->maxJunctionSpeed); // possibly unneeded???
      }
      // If actual line start speed has not been updated to maximum speed then
      // do it now
      if (act->startSpeed != previous->maxJunctionSpeed) {
        act->startSpeed = RMath::max(
            act->minSpeed, previous->maxJunctionSpeed); // possibly unneeded???
        act->invalidateParameter();
      }
      lastJunctionSpeed = previous->endSpeed;
    } else {
      // Block previous end and act start as calculated speed and recalculate
      // plateau speeds (which could move the speed higher again)
      act->startSpeed = RMath::max(act->minSpeed, lastJunctionSpeed);
      lastJunctionSpeed = previous->endSpeed =
          RMath::max(lastJunctionSpeed, previous->minSpeed);
      previous->invalidateParameter();
      act->invalidateParameter();
    }
    act = previous;
...
void PrintLine::forwardPlanner(ufast8_t first) {
  ...
  while (first !=
         linesWritePos) { // All except last segment, which has fixed end speed
    act = next;
    nextPlannerIndex(first);
    next = &lines[first];
...
#if NONLINEAR_SYSTEM
    /*        if (act->moveID == next->moveID && act->endSpeed ==
       act->maxJunctionSpeed)
            {
    ...
    vmaxRight =
        (act->isNominalMove()
             ? act->fullSpeed
             : sqrt(leftSpeed * leftSpeed + act->accelerationDistance2));
    if (vmaxRight > act->endSpeed) { // Could be higher next run?
      ...
      act->startSpeed = leftSpeed;
      next->startSpeed = leftSpeed = RMath::max(
          RMath::min(act->endSpeed, act->maxJunctionSpeed), next->minSpeed);
      if (act->endSpeed ==
          act->maxJunctionSpeed) { // Full speed reached, don't compute again!
        act->setEndSpeedFixed(true);
        next->setStartSpeedFixed(true);
      }
      act->invalidateParameter();
    } else { // We can accelerate full speed without reaching limit, which is as
             // fast as possible. Fix it!
  ...
  next->startSpeed = RMath::max(
      next->minSpeed, leftSpeed); // This is the new segment, which is updated
                                  // anyway, no extra flag needed.
}
...
inline float PrintLine::safeSpeed(fast8_t drivingAxis) {
  ...
  if (isEMove()) {
    if (isXYZMove())
      safe = RMath::min(safe, 0.5 * Extruder::current->maxStartFeedrate *
                                  fullSpeed / fabs(speedE));
    else
      safe = 0.5 *
             Extruder::current->maxStartFeedrate; // This is a retraction move
  }
  // Check for minimum speeds needed for numerical robustness
#if DRIVE_SYSTEM == DELTA
  if (drivingAxis == X_AXIS || drivingAxis == Y_AXIS ||
      drivingAxis == Z_AXIS) // enforce minimum speed for numerical stability of
                             // explicit speed integration
    safe = RMath::max(xyMin, safe);
#else
  if (drivingAxis == X_AXIS ||
      drivingAxis == Y_AXIS) // enforce minimum speed for numerical stability of
                             // explicit speed integration
    safe = RMath::max(xyMin, safe);
...
/** Check if move is new. If it is insert some dummy moves to allow the path
optimizer to work since it does not act on the first two moves in the queue. The
stepper timer will spot these moves and leave some time for processing.
*/
uint8_t PrintLine::insertWaitMovesIfNeeded(uint8_t pathOptimize,
                                           uint8_t waitExtraLines) {
  if (linesCount == 0 && waitRelax == 0 &&
      pathOptimize) { // First line after some time - warm up needed
                      // return 0;
#if NONLINEAR_SYSTEM
    uint8_t w = 3;
#else
...
void PrintLine::waitForXFreeLines(uint8_t b, bool allowMoves) {
  while (getLinesCount() + b >
         PRINTLINE_CACHE_SIZE) { // wait for a free entry in movement cache
...
#ifdef FAST_COREXYZ
uint8_t transformCartesianStepsToDeltaSteps(int32_t cartesianPosSteps[],
                                            int32_t corePosSteps[]) {
...
/**
  ...
  @returns 1 if Cartesian coordinates have a valid delta tower position 0 if
  not.
*/
uint8_t transformCartesianStepsToDeltaSteps(int32_t cartesianPosSteps[],
                                            int32_t deltaPosSteps[]) {
  ...
  if (cnt >= DISTORTION_UPDATE_FREQUENCY || lastZSteps != zSteps) {
    cnt = 0;
    lastZSteps = zSteps;
    lastZCorrection = Printer::distortion.correct(cartesianPosSteps[X_AXIS],
                                                  cartesianPosSteps[Y_AXIS],
  ...
  if (Printer::isLargeMachine()) {
    ...
    // A TOWER height
    uint64_t temp =
        RMath::absLong(Printer::deltaAPosYSteps - cartesianPosSteps[Y_AXIS]);
    ...
    if (deltaPosSteps[A_TOWER] < Printer::deltaFloorSafetyMarginSteps &&
        !Printer::isZProbingActive())
    ...
    if (deltaPosSteps[B_TOWER] < Printer::deltaFloorSafetyMarginSteps &&
        !Printer::isZProbingActive())
    ...
    if (deltaPosSteps[C_TOWER] < Printer::deltaFloorSafetyMarginSteps &&
        !Printer::isZProbingActive())
    ...
    // As we are right on the edge of many printers arm lengths, this is
    // rewritten to use unsigned long This allows 52% longer arms to be used
    // without performance penalty the code is a bit longer, because we cannot
    // use negative to test for invalid conditions Also, previous code did not
    // check for overflow of squared result Overflow is also detected as a fault
    // condition
    ...
    /*
            ...
            if ((temp = opt - temp2 * temp2) >= 0)
                ...
                deltaPosSteps[A_TOWER] = HAL::integerSqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #else deltaPosSteps[A_TOWER] = sqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #endif else return 0;
            ...
            if ((temp = opt - temp2*temp2) >= 0)
    #ifdef FAST_INTEGER_SQRT
                deltaPosSteps[B_TOWER] = HAL::integerSqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #else deltaPosSteps[B_TOWER] = sqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #endif else return 0;
            ...
            if ((temp = opt - temp2*temp2) >= 0)
    #ifdef FAST_INTEGER_SQRT
                deltaPosSteps[C_TOWER] = HAL::integerSqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #else deltaPosSteps[C_TOWER] = sqrt(temp) +
    cartesianPosSteps[Z_AXIS]; #endif else return 0;*/
  }
  return 1;
}
...
#endif
#if DRIVE_SYSTEM == DELTA
    if (isZPositiveMove() && isXPositiveMove() && isYPositiveMove() &&
        Endstops::anyXYZMax())
#else
...
void PrintLine::calculateDirectionAndDelta(float fdifference[],
                                           int32_t difference[], ufast8_t *dir,
                                           int32_t delta[]) {
  *dir = 0;
  // Find direction
...
inline uint16_t PrintLine::calculateNonlinearSubSegments(uint8_t softEndstop) {
  ...
  for (int i = 0; i < Z_AXIS_ARRAY; i++)
    dx[i] = static_cast<float>(Printer::destinationSteps[i] -
                               Printer::currentPositionSteps[i]) /
            static_cast<float>(numNonlinearSegments);
#endif
//  out.println_byte_P(PSTR("Calculate delta segments:"), p->numDeltaSegments);
  ...
  for (int s = numNonlinearSegments; s > 0; s--) {
    ...
    for (i = 0; i < Z_AXIS_ARRAY; i++) {
      ...
      // This method generates small waves which get larger with increasing
      // number of delta segments. smaller?
    ...
    for (i = 0; i < Z_AXIS_ARRAY; i++) // End of segment in Cartesian steps
      // Perfect approximation, but slower, so we limit it to faster processors
      // like arm
      destinationSteps[i] =
          lroundf(dx[i] * segment) + Printer::currentPositionSteps[i];
#endif
    // Verify that delta calculation has a solution
    if (transformCartesianStepsToDeltaSteps(destinationSteps,
                                            destinationDeltaSteps)) {
      d->dir = 0;
#if DRIVE_SYSTEM == DELTA
      if (softEndstop) {
        destinationDeltaSteps[A_TOWER] = RMath::min(
            destinationDeltaSteps[A_TOWER], Printer::maxDeltaPositionSteps);
        destinationDeltaSteps[B_TOWER] = RMath::min(
            destinationDeltaSteps[B_TOWER], Printer::maxDeltaPositionSteps);
        destinationDeltaSteps[C_TOWER] = RMath::min(
            destinationDeltaSteps[C_TOWER], Printer::maxDeltaPositionSteps);
      }
#endif
      for (i = 0; i < TOWER_ARRAY; i++) {
        delta = destinationDeltaSteps[i] -
                Printer::currentNonlinearPositionSteps[i];
        if (delta > 0) {
      ...
      d->dir = 0;
      d->deltaSteps[A_TOWER] = d->deltaSteps[B_TOWER] = d->deltaSteps[C_TOWER] =
          0;
      return 65535; // flag error
    }
  }
#ifdef DEBUG_STEPCOUNT
//      out.println_long_P(PSTR("initial StepsRemaining:"),
//      p->totalStepsRemaining);
#endif
  return maxAxisSteps;
}

uint8_t PrintLine::calculateDistance(float axisDistanceMM[], uint8_t dir,
                                     float *distance) {
...
void PrintLine::calculatePlane(int32_t factors[], int32_t p1[], int32_t p2[],
                               int32_t p3[]) {
  factors[0] = p1[1] * (p2[2] - p3[2]) + p2[1] * (p3[2] - p1[2]) +
               p3[1] * (p1[2] - p2[2]);
  factors[1] = p1[2] * (p2[0] - p3[0]) + p2[2] * (p3[0] - p1[0]) +
               p3[2] * (p1[0] - p2[0]);
  factors[2] = p1[0] * (p2[1] - p3[1]) + p2[0] * (p3[1] - p1[1]) +
               p3[0] * (p1[1] - p2[1]);
  factors[3] = p1[0] * ((p2[1] * p3[2]) - (p3[1] * p2[2])) +
               p2[0] * ((p3[1] * p1[2]) - (p1[1] * p3[2])) +
               p3[0] * ((p1[1] * p2[2]) - (p2[1] * p1[2]));
}

float PrintLine::calcZOffset(int32_t factors[], int32_t pointX,
                             int32_t pointY) {
  return (factors[3] - factors[X_AXIS] * pointX - factors[Y_AXIS] * pointY) /
         (float)factors[2];
}
#endif

inline void PrintLine::queueEMove(int32_t extrudeDiff, uint8_t check_endstops,
                                  uint8_t pathOptimize) {
  Printer::unsetAllSteppersDisabled();
  waitForXFreeLines(1);
  ...
  axisDistanceMM[E_AXIS] = p->distance =
      p->delta[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS];
...
/**
  Split a line up into a series of lines with at most
  DELTASEGMENTS_PER_PRINTLINE delta segments.
...
*/
uint8_t PrintLine::queueNonlinearMove(uint8_t check_endstops,
                                      uint8_t pathOptimize,
                                      uint8_t softEndstop) {
  ENSURE_POWER
  // if (softEndstop && Printer::destinationSteps[Z_AXIS] < 0)
  // Printer::destinationSteps[Z_AXIS] = 0; // now constrained at entry level
  // including cylinder test
  EVENT_CONTRAIN_DESTINATION_COORDINATES
  int32_t difference[E_AXIS_ARRAY];
  float axisDistanceMM[VIRTUAL_AXIS_ARRAY]; // Real cartesian axis movement in
                                            // mm. Virtual axis in 4;
  secondspeed_t secondSpeed = Printer::fanSpeed;
  for (fast8_t axis = 0; axis < E_AXIS_ARRAY; axis++) {
    difference[axis] =
        Printer::destinationSteps[axis] - Printer::currentPositionSteps[axis];
    // axisDistanceMM[axis] = fabs(difference[axis] *
    // Printer::invAxisStepsPerMM[axis]);
    if (axis == E_AXIS) {
      axisDistanceMM[E_AXIS] =
          (Printer::destinationPositionTransformed[E_AXIS] -
           Printer::currentPositionTransformed[E_AXIS]) *
      ...
      if (Printer::mode == PRINTER_MODE_FFF) {
        Printer::extrudeMultiplyError +=
            axisDistanceMM[axis] * Printer::axisStepsPerMM[E_AXIS];
        difference[E_AXIS] =
            static_cast<int32_t>(Printer::extrudeMultiplyError);
        Printer::extrudeMultiplyError -= difference[E_AXIS];
        axisDistanceMM[E_AXIS] =
            difference[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS];
        ...
        secondSpeed =
            ((axisDistanceMM[X_AXIS] != 0 || axisDistanceMM[Y_AXIS] != 0) &&
                     (LaserDriver::laserOn || axisDistanceMM[E_AXIS] != 0)
                 ? LaserDriver::intensity
                 : 0);
  ...
  calculateDirectionAndDelta(axisDistanceMM, difference, &cartesianDir,
                             cartesianDeltaSteps);
...
#else
  float feedrate = Printer::feedrate; // each motor has own max. feedrate here
                                      // resulting in total feedrate
#endif
  if (cartesianDir & XY_STEP) {
    // Compute number of seconds for move and hence number of segments needed
    // float seconds = 100 * cartesianDistance / (Printer::feedrate *
    // Printer::feedrateMultiply); multiply in feedrate included
    float seconds = cartesianDistance / feedrate;
#ifdef DEBUG_SPLIT
    Com::printFLN(Com::tDBGDeltaSeconds, seconds);
#endif
    float sps = static_cast<float>((cartesianDir & ESTEP) == ESTEP
                                       ? Printer::printMovesPerSecond
                                       : Printer::travelMovesPerSecond);
    ...
    // Optimize pure Z axis move. Since a pure Z axis move is linear all we have
    // to watch out for is unsigned integer overruns in the queued moves;
#ifdef DEBUG_SPLIT
    Com::printFLN(Com::tDBGDeltaZDelta, cartesianDeltaSteps[Z_AXIS]);
#endif
    segmentCount = (cartesianDeltaSteps[Z_AXIS] + (uint32_t)43680) /
                   (uint32_t)43679; // can not go to 65535 for rounding issues
                                    // causing overflow later in some cases!
  }
  // Now compute the number of lines needed
  int numLines = (segmentCount + DELTASEGMENTS_PER_PRINTLINE - 1) /
                 DELTASEGMENTS_PER_PRINTLINE;
  // There could be some error here but it doesn't matter since the number of
  // segments will just be reduced slightly
  ...
  insertWaitMovesIfNeeded(pathOptimize,
                          RMath::min(PRINTLINE_CACHE_SIZE - 4, numLines));
  uint32_t oldEDestination =
      Printer::destinationSteps[E_AXIS]; // flow and volumetric extrusion
                                         // changed virtual target
  ...
  for (int lineNumber = 1; lineNumber <= numLines; lineNumber++) {
    waitForXFreeLines(1);
    PrintLine *p = getNextWriteLine();
    // Downside a comparison per loop. Upside one less distance calculation and
    // simpler code.
    if (numLines == 1) {
      // p->numDeltaSegments = segmentCount; // not neede, gets overwritten
      // further down
      p->dir = cartesianDir;
      for (fast8_t i = 0; i < E_AXIS_ARRAY; i++) {
      ...
      for (fast8_t i = 0; i < E_AXIS_ARRAY; i++) {
        Printer::destinationSteps[i] =
            startPosition[i] + (difference[i] * lineNumber) / numLines;
        fractionalSteps[i] =
            Printer::destinationSteps[i] - Printer::currentPositionSteps[i];
        if ((cartesianDir & (16 << i)) == 0) {
          axisDistanceMM[i] = -axisDistanceMM[i]; // restore direction for
                                                  // calculateDirectionAndDelta
        }
        // axisDistanceMM[i] = fabs(fractionalSteps[i] *
        // Printer::invAxisStepsPerMM[i]);
      }
      calculateDirectionAndDelta(axisDistanceMM, fractionalSteps, &p->dir,
                                 p->delta);
      p->distance = cartesianDistance;
    }
    ...
    if (maxStepsPerSegment == 65535) {
      Com::printWarningFLN(PSTR(
          "in queueDeltaMove to calculateDeltaSubSegments returns error."));
      return false;
    }
#ifdef DEBUG_SPLIT
    Com::printFLN(Com::tDBGDeltaMaxDS, (int32_t)maxStepsPerSegment);
#endif
    int32_t virtualAxisSteps =
        static_cast<int32_t>(maxStepsPerSegment) * segmentsPerLine;
    ...
    p->primaryAxis =
        VIRTUAL_AXIS; // Virtual axis will lead Bresenham step either way
    if (virtualAxisSteps >
        p->delta[E_AXIS]) { // Is delta move or E axis leading
      p->stepsRemaining = virtualAxisSteps;
      axisDistanceMM[VIRTUAL_AXIS] =
          p->distance; // virtual_axis_move *
                       // Printer::invAxisStepsPerMM[Z_AXIS]; // Steps/unit same
                       // as all the towers
      // Virtual axis steps per segment
      p->numPrimaryStepPerSegment = maxStepsPerSegment;
#if DRIVE_SYSTEM != DELTA
      if (cartesianDeltaSteps[Z_AXIS] > cartesianDeltaSteps[X_AXIS] &&
          cartesianDeltaSteps[Z_AXIS] > cartesianDeltaSteps[Y_AXIS])
        drivingAxis = Z_AXIS;
#endif
    } else {
      // Round up the E move to get something divisible by segment count which
      // is greater than E move
      p->numPrimaryStepPerSegment =
          (p->delta[E_AXIS] + segmentsPerLine - 1) / segmentsPerLine;
      p->stepsRemaining = p->numPrimaryStepPerSegment * segmentsPerLine;
      axisDistanceMM[VIRTUAL_AXIS] =
          -p->distance; // p->stepsRemaining *
                        // Printer::invAxisStepsPerMM[Z_AXIS];
      drivingAxis = E_AXIS;
    }
#ifdef DEBUG_SPLIT
  ...
  for (fast8_t i = 0; i <= E_AXIS; i++) {
    Printer::currentPositionTransformed[i] =
        Printer::destinationPositionTransformed[i];
  }
...
void PrintLine::arc(float *position, float *target, float *offset, float radius,
                    uint8_t isclockwise) {
  //   int acceleration_manager_was_enabled =
  //   plan_is_acceleration_manager_enabled();
  //   plan_set_acceleration_manager_enabled(false); // disable acceleration
  //   management for the duration of the arc
  ...
  float extruder_travel = (Printer::destinationSteps[E_AXIS] -
  ...
  */
  ...
  if ((!isclockwise && angular_travel <= 0.00001) ||
      (isclockwise && angular_travel < -0.000001)) {
  ...
  float millimeters_of_travel =
      fabs(angular_travel) *
      radius; // hypot(angular_travel*radius, fabs(linear_travel));
  if (millimeters_of_travel < 0.001f) {
    return; // treat as succes because there is nothing to do;
  }
  // uint16_t segments = (radius>=BIG_ARC_RADIUS ?
  // floor(millimeters_of_travel/MM_PER_ARC_SEGMENT_BIG) :
  // floor(millimeters_of_travel/MM_PER_ARC_SEGMENT));
  // Increase segment size if printing faster then computation speed allows
  uint16_t segments =
      (Printer::feedrate > 60.0f
           ? floor(millimeters_of_travel /
                   RMath::min(static_cast<float>(MM_PER_ARC_SEGMENT_BIG),
                              Printer::feedrate * 0.01666f *
                                  static_cast<float>(MM_PER_ARC_SEGMENT)))
           : floor(millimeters_of_travel /
                   static_cast<float>(MM_PER_ARC_SEGMENT)));
  if (segments == 0)
    segments = 1;
  /*
    ...
    is approximated
    // by a number of discrete segments. The inverse feed_rate should be correct
    for the sum of
    // all segments.
  ...
  */
  ...
  /* Vector rotation by transformation matrix: r is the original vector, r_T is
     ...
     into the planner without the initial overhead of computing cos() or sin().
     By the time the arc needs to be applied a correction, the planner should
     have caught up to the lag caused by the initial mc_arc overhead. This is
     important when there are successive arc motions.
  */
  ...
  arc_target[E_AXIS] = Printer::currentPositionSteps[E_AXIS] *
                       Printer::invAxisStepsPerMM[E_AXIS];
  ...
  for (i = 1; i < segments; i++) {
    ...
    Printer::moveToReal(arc_target[X_AXIS], arc_target[Y_AXIS],
                        IGNORE_COORDINATE, arc_target[E_AXIS],
                        IGNORE_COORDINATE);
  }
  // Ensure last segment arrives at target location.
  Printer::moveToReal(target[X_AXIS], target[Y_AXIS], IGNORE_COORDINATE,
                      target[E_AXIS], IGNORE_COORDINATE);
}
#endif

/**
  Moves the stepper motors one step. If the last step is reached, the next
  movement is started. The function must be called from a timer loop. It returns
  the time for the next call. This is a modified version that implements a
  Bresenham 'multi-step' algorithm where the dominant Cartesian axis steps may
  be less than the changing dominant delta axis.
*/
#if NONLINEAR_SYSTEM
int lastblk = -1;
int32_t cur_errupd;
...
int32_t PrintLine::bresenhamStep() { // Version for delta printer
...
#endif
  {
...
#endif
    if (cur->isWarmUp()) {
      // This is a warm up move to initialize the path planner correctly. Just
      // waste a bit of time to get the planning up to date.
...
#if FEATURE_Z_PROBE
    // z move may consist of more then 1 z line segment, so we better ignore
    // them if the probe was already hit.
    ...
    if (cur->numNonlinearSegments) {
      ...
      // Enable axis - All axis are enabled since they will most probably all be
      // involved in a move Since segments could involve different axis this
      // reduces load when switching segments and makes disabling easier.
      ...
      // Copy across movement into main direction flags so that endstops
      // function correctly
#if DRIVE_SYSTEM == DELTA
      cur->dir |= curd->dir; // deltas need this for homing!
#endif
      // Initialize Bresenham for the first segment
      cur->error[X_AXIS] = cur->error[Y_AXIS] = cur->error[Z_AXIS] =
          cur->numPrimaryStepPerSegment >> 1;
    ...
    if (!cur->areParameterUpToDate()) { // should never happen, but with bad
                                        // timings???
...
#if USE_ADVANCE
    if (!Printer::isAdvanceActivated()) // Set direction if no advance/OPS
                                        // enabled
#endif
      Extruder::setDirection(cur->isEPositiveMove());
#if defined(DIRECTION_DELAY) && DIRECTION_DELAY > 0
      // HAL::delayMicroseconds(DIRECTION_DELAY); // We leave interrupt without
      // step so no delay needed here
#endif
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
    ...
    Printer::multiXHomeFlags =
        MULTI_XENDSTOP_ALL; // move all x motors until endstop says differently
#endif
#if MULTI_YENDSTOP_HOMING
    Printer::multiYHomeFlags =
        MULTI_YENDSTOP_ALL; // move all y motors until endstop says differently
#endif
#if MULTI_ZENDSTOP_HOMING
    Printer::multiZHomeFlags =
        MULTI_ZENDSTOP_ALL; // move all z motors until endstop says differently
#endif
    return Printer::interval; // Wait an other 50% from last step to make the
                              // 100% full
  }                           // End cur=0
  HAL::allowInterrupts();

  if (curd != NULL) {
    if (curd->checkEndstops(cur,
                            (cur->isCheckEndstops()))) { // should stop move
      cur->stepsRemaining = 0;
      curd = NULL;
...
#if defined(SUPPORT_LASER) && SUPPORT_LASER
      else if (Printer::mode ==
               PRINTER_MODE_LASER) { // Last move disables laser for safety!
  ...
  int maxLoops = (Printer::stepsPerTimerCall <= cur->stepsRemaining
                      ? Printer::stepsPerTimerCall
                      : cur->stepsRemaining);
  HAL::forbidInterrupts();
  for (int loop = 0; loop < maxLoops; loop++) {
    ...
    if (curd) {
      ...
      if (curd->isXMove())
        if ((cur->error[X_AXIS] -= curd->deltaSteps[A_TOWER]) < 0) {
          ...
          Printer::realDeltaPositionSteps[A_TOWER] +=
              curd->isXPositiveMove() ? 1 : -1;
#endif
#ifdef DEBUG_STEPCOUNT
...
#ifdef DEBUG_REAL_POSITION
          Printer::realDeltaPositionSteps[B_TOWER] +=
              curd->isYPositiveMove() ? 1 : -1;
#endif
#ifdef DEBUG_STEPCOUNT
          cur->totalStepsRemaining--;
#endif
      ...
      if (curd->isZMove())
        if ((cur->error[Z_AXIS] -= curd->deltaSteps[C_TOWER]) < 0) {
          ...
          Printer::realDeltaPositionSteps[C_TOWER] +=
              curd->isZPositiveMove() ? 1 : -1;
#ifdef DEBUG_STEPCOUNT
          cur->totalStepsRemaining--;
#endif
        }
    ...
    if (!stepsPerSegRemaining) { // start new nonlinear segment
      if (cur->numNonlinearSegments && curd != NULL) {
        ...
        // Initialize Bresenham for this segment (numPrimaryStepPerSegment is
        // already correct for the half step setting)
        cur->error[X_AXIS] = cur->error[Y_AXIS] = cur->error[Z_AXIS] =
            cur->numPrimaryStepPerSegment >> 1;

        // Reset the counter of the primary steps. This is initialized in the
        // line generation so don't have to do this the first time.
        stepsPerSegRemaining = cur->numPrimaryStepPerSegment;
  ...
  HAL::allowInterrupts(); // Allow interrupts for other types, timer1 is still
                          // disabled
#if RAMP_ACCELERATION
  // If acceleration is enabled on this move and we are in the acceleration
  // segment, calculate the current interval
  if (cur->moveAccelerating()) {
    Printer::vMaxReached =
        HAL::ComputeV(Printer::timer, cur->fAcceleration) + cur->vStart;
    ...
    // if(Printer::maxInterval < Printer::interval) // fix timing for very slow
    // speeds
    ...
    if (v > Printer::vMaxReached) // if deceleration goes too far it can become
                                  // too large
      v = cur->vEnd;
    else {
      v = Printer::vMaxReached - v;
      if (v < cur->vEnd)
        v = cur->vEnd; // extra steps at the end of deceleration due to rounding
                       // errors
    }
    ...
    // If we had acceleration, we need to use the latest vMaxReached and
    // interval If we started full speed, we need to use cur->fullInterval and
    // vMax
    cur->updateAdvanceSteps(
        (!cur->accelSteps ? cur->vMax : Printer::vMaxReached), 0, true);
    if (!cur->accelSteps) {
...
int32_t PrintLine::bresenhamStep() { // version for Cartesian printer
...
#endif
  {
    ...
    // Only enable axis that are moving. If the axis doesn't need to move then
    // it can stay disabled depending on configuration.
#if GANTRY
    ...
    long gdx =
        (cur->dir & X_DIRPOS
             ? cur->delta[X_AXIS]
             : -cur->delta[X_AXIS]); // Compute signed difference in steps
#if DRIVE_SYSTEM == XY_GANTRY || DRIVE_SYSTEM == YX_GANTRY
    Printer::setZDirection(cur->isZPositiveMove());
  ...
  if (cur->moveAccelerating()) { // we are accelerating
    Printer::vMaxReached = HAL::ComputeV(Printer::timer, cur->fAcceleration) +
                           cur->vStart; // v = v0 + a * t
    if (Printer::vMaxReached > cur->vMax) {
      Printer::vMaxReached = cur->vMax;
    }
  ...
  if (cur->stepsRemaining <= 0 || cur->isNoMove()) { // line finished
    ...
    interval = Printer::interval =
        interval >> 1; // 50% of time to next call to do cur=0
    DEBUG_MEMORY;
  } // Do even
#if FEATURE_BABYSTEPPING
