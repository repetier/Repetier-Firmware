...
class SerialGCodeSource : public GCodeSource {
    ...
    uint8_t commandsReceivingWritePosition; ///< Writing position in
                                            ///< gcode_transbuffer.
    uint8_t sendAsBinary;                   ///< Flags the command as binary input.
    uint8_t commentDetected;                ///< Flags true if we are reading the comment part of
                                            ///< a command.
...
public:
    ...
    virtual bool closeOnError();  // return true if the channel can not
...
public:
    ...
    // Old habit from PC, which require alignments for data types such as int and
    // long to be on 2 or 4 byte boundary Otherwise, the compiler adds padding,
    // wasted space.
    uint8_t
        T; // This may not matter on any of these controllers, but it can't hurt
    // True if origin did not come from serial console. That way we can send
    // status messages to a host only if he would normally not know about the mode
    // switch.
    ...
    void echoCommand();
    /** Get next command in command buffer. After the command is processed, call
...
protected:
    ...
    static GCode
        commandsBuffered[GCODE_BUFFER_SIZE];       ///< Buffer for received commands.
    ...
    static uint8_t commandsReceivingWritePosition; ///< Writing position in
                                                   ///< gcode_transbuffer.
    static uint8_t sendAsBinary;                   ///< Flags the command as binary input.
    static uint8_t commentDetected;                ///< Flags true if we are reading the comment
                                                   ///< part of a command.
    static uint8_t
        binaryCommandSize; ///< Expected size of the incoming binary command.
    static bool
        waitUntilAllCommandsAreParsed; ///< Don't read until all commands are
                                       ///< parsed. Needed if gcode_buffer is
                                       ///< misused as storage for strings.
    static uint32_t actLineNumber;     ///< Line number of current command.
    static volatile uint8_t
        bufferLength;               ///< Number of commands stored in gcode_buffer
    ...
    static int8_t
        waitingForResend;           ///< Waiting for line to be resend. -1 = no wait.
    static uint32_t lastLineNumber; ///< Last line number received.
    static millis_t
        timeOfLastDataPacket;                      ///< Time, when we got the last data packet. Used to
                                                   ///< detect missing uint8_ts.
    static uint8_t wasLastCommandReceivedAsBinary; ///< Was the last successful
