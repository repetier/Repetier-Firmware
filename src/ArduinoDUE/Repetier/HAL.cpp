...
void HAL::setupTimer() {
  ...
  TC_SetRC(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL,
           (F_CPU_TRUE / 32) /
               EXTRUDER_CLOCK_FREQ); // set frequency 43 for 60000Hz
...
static PWMChannel pwm_channel[8] = {{false, nullptr, 0}, {false, nullptr, 0},
                                    {false, nullptr, 0}, {false, nullptr, 0},
                                    {false, nullptr, 0}, {false, nullptr, 0},
                                    {false, nullptr, 0}, {false, nullptr, 0}};
...
void HAL::analogStart(void) {
  ...
  for (int i = 0; i < ANALOG_INPUTS; i++) {
    ...
    for (int j = 0; j < ANALOG_INPUT_MEDIAN; j++)
      osAnalogSamples[i][j] =
          2048; // we want to prevent early error from bad starting values
  }
  // enable channels
  ...
  ADC->ADC_MR =
      ADC_MR_TRGEN_DIS | ADC_MR_TRGSEL_ADC_TRIG0 | ADC_MR_LOWRES_BITS_12 |
      ADC_MR_SLEEP_NORMAL | ADC_MR_FWUP_OFF | ADC_MR_FREERUN_OFF |
      ADC_MR_STARTUP_SUT64 | ADC_MR_SETTLING_AST17 | ADC_MR_ANACH_NONE |
      ADC_MR_USEQ_NUM_ORDER | ADC_MR_PRESCAL(AD_PRESCALE_FACTOR) |
      ADC_MR_TRACKTIM(AD_TRACKING_CYCLES) | ADC_MR_TRANSFER(AD_TRANSFER_CYCLES);
...
void HAL::syncEEPROM() { // store to disk if changed
  ...
  if (eprSyncTime &&
      (time - eprSyncTime >
       15000)) { // Buffer writes only every 15 seconds to pool writes
    eprSyncTime = 0;
    bool failed = false;
...
void HAL::spiInit(uint8_t spiClock) {
  ...
  if (spiInitMaded == false) {
    ...
    SPI_ConfigureNPCS(SPI0, SPI_CHAN_EEPROM1,
                      SPI_CSR_NCPHA | SPI_CSR_CSAAT |
                          SPI_CSR_SCBR(spiDueDividors[spiClock]) |
                          SPI_CSR_DLYBCT(1));
#endif // MOTHERBOARD==500 || MOTHERBOARD==501 || (MOTHERBOARD==502)

    // Set SPI mode 0, clock, select not active after transfer, with delay
    // between transfers
    SPI_ConfigureNPCS(SPI0, SPI_CHAN,
                      SPI_CSR_NCPHA | SPI_CSR_CSAAT |
                          SPI_CSR_SCBR(spiDueDividors[spiClock]) |
                          SPI_CSR_DLYBCT(1));
    SPI_Enable(SPI0);
...
unsigned char HAL::i2cStart(unsigned char address_and_direction) {
  ...
  TWI_INTERFACE->TWI_MMR =
      (twiDirection << 12) | TWI_MMR_IADRSZ_NONE | TWI_MMR_DADR(address);
  TWI_INTERFACE->TWI_CR =
      TWI_CR_MSEN | TWI_CR_SVDIS; // set master mode disable slave mode
  if (twiDirection)
...
void HAL::i2cStartAddr(unsigned char address_and_direction, unsigned int pos) {
  ...
  TWI_INTERFACE->TWI_MMR = 0;
  TWI_INTERFACE->TWI_MMR =
      (twiDirection << 12) | EEPROM_ADDRSZ_BYTES | TWI_MMR_DADR(address);
...
void HAL::servoMicroseconds(uint8_t servo, int microsec, uint16_t autoOff) {
  ...
  if (microsec > 2500)
    microsec = 2500;
  servoTimings[servo] =
      (unsigned int)(((F_CPU_TRUE / SERVO_PRESCALE) / 1000000) * microsec);
...
void TIMER1_COMPA_VECTOR() {
  ...
  if (stepperChannel->TC_CV + STEPPERTIMER_EXIT_TICKS > timer_count) {
    stepperChannel->TC_RC =
        stepperChannel->TC_CV +
        STEPPERTIMER_EXIT_TICKS; // should end after exiting timer interrupt
...
void PWM_TIMER_VECTOR() {
  ...
  if (pwm_count_cooler == 0 && !PDM_FOR_COOLER) {
    ...
    EXT1_EXTRUDER_COOLER_PIN != EXT0_EXTRUDER_COOLER_PIN
    ...
    if ((pwm_pos_set[PWM_BOARD_FAN] =
             (pwm_pos[PWM_BOARD_FAN] & COOLER_PWM_MASK)) > 0)
    ...
    if ((pwm_pos_set[PWM_FAN_THERMO] =
             (pwm_pos[PWM_FAN_THERMO] & COOLER_PWM_MASK)) > 0)
...
#else
  if (pwm_cooler_pos_set[0] == pwm_count_cooler &&
      pwm_cooler_pos_set[0] != COOLER_PWM_MASK)
  ...
  if (pwm_pos_set[1] == pwm_count_heater && pwm_pos_set[1] != HEATER_PWM_MASK)
    WRITE(EXT1_HEATER_PIN, HEATER_PINS_INVERTED);
#endif
#if !SHARED_COOLER && defined(EXT1_EXTRUDER_COOLER_PIN) &&                     \
    EXT1_EXTRUDER_COOLER_PIN > -1 &&                                           \
    EXT1_EXTRUDER_COOLER_PIN != EXT0_EXTRUDER_COOLER_PIN
#if PDM_FOR_COOLER
  pulseDensityModulate(EXT1_EXTRUDER_COOLER_PIN, extruder[1].coolerPWM,
                       pwm_cooler_pos_set[1], false);
#else
  if (pwm_cooler_pos_set[1] == pwm_count_cooler &&
      pwm_cooler_pos_set[1] != COOLER_PWM_MASK)
...
#else
  if (pwm_cooler_pos_set[2] == pwm_count_cooler &&
      pwm_cooler_pos_set[2] != COOLER_PWM_MASK)
...
#else
  if (pwm_cooler_pos_set[3] == pwm_count_cooler &&
      pwm_cooler_pos_set[3] != COOLER_PWM_MASK)
...
#else
  if (pwm_cooler_pos_set[4] == pwm_count_cooler &&
      pwm_cooler_pos_set[4] != COOLER_PWM_MASK)
...
#else
  if (pwm_cooler_pos_set[5] == pwm_count_cooler &&
      pwm_cooler_pos_set[5] != COOLER_PWM_MASK)
...
#else
  if (pwm_pos_set[PWM_BOARD_FAN] == pwm_count_cooler &&
      pwm_pos_set[PWM_BOARD_FAN] != COOLER_PWM_MASK)
  ...
  if (fanKickstart == 0) {
    ...
    pulseDensityModulate(FAN_PIN, pwm_pos[PWM_FAN1], pwm_pos_set[PWM_FAN1],
                         false);
#else
    if (pwm_pos_set[PWM_FAN1] == pwm_count_cooler &&
        pwm_pos_set[PWM_FAN1] != COOLER_PWM_MASK)
      WRITE(FAN_PIN, 0);
#endif
  } else {
#if PDM_FOR_COOLER
...
#else
    if ((MAX_FAN_PWM & COOLER_PWM_MASK) == pwm_count_cooler &&
        (MAX_FAN_PWM & COOLER_PWM_MASK) != COOLER_PWM_MASK)
      WRITE(FAN_PIN, 0);
#endif
  }
#endif
  ...
  if (fan2Kickstart == 0) {
    ...
    pulseDensityModulate(FAN2_PIN, pwm_pos[PWM_FAN2], pwm_pos_set[PWM_FAN2],
                         false);
#else
    if (pwm_pos_set[PWM_FAN2] == pwm_count_cooler &&
        pwm_pos_set[PWM_FAN2] != COOLER_PWM_MASK)
      WRITE(FAN2_PIN, 0);
#endif
  } else {
#if PDM_FOR_COOLER
...
#else
  if (pwm_pos_set[PWM_FAN_THERMO] == pwm_count_cooler &&
      pwm_pos_set[PWM_FAN_THERMO] != COOLER_PWM_MASK)
...
#else
  if (pwm_pos_set[NUM_EXTRUDER] == pwm_count_heater &&
      pwm_pos_set[NUM_EXTRUDER] != HEATER_PWM_MASK)
    WRITE(HEATED_BED_HEATER_PIN, HEATER_PINS_INVERTED);
#endif
#endif
  ...
  if ((ADC->ADC_ISR & adcEnable) == adcEnable) {
    ...
    for (int i = 0; i < ANALOG_INPUTS; i++) {
      ...
      if (adcCounter >= NUM_ADC_SAMPLES) { // store new conversion result
        ...
        osAnalogInputBuildup[i] = osAnalogInputBuildup[i] +
                                  (1 << (ANALOG_INPUT_SAMPLE - 1)) -
                                  (adcSamplesMin[i] + adcSamplesMax[i]);
        adcSamplesMin[i] = 100000;
        adcSamplesMax[i] = 0;
        osAnalogSamplesSum[i] -= osAnalogSamples[i][adcSamplePos];
        osAnalogSamplesSum[i] +=
            (osAnalogSamples[i][adcSamplePos] =
                 osAnalogInputBuildup[i] >> ANALOG_INPUT_SAMPLE);
...
*/
#if USE_ADVANCE
TcChannel *extruderChannel =
    (EXTRUDER_TIMER->TC_CHANNEL + EXTRUDER_TIMER_CHANNEL);
